...
Local nPrazoAnt	    := GetNewPar("FS_DEL050",180)  // Prazo de Verificacao de Doacao com data Anterior a original
Local nTotQAgreg    := 0
Local nTotQDoa      := 0
Local _nQtd         := 0
Local _cItem        := ""
...
//		Aadd(aDadosDoa,{ "D" , TRB->UA_EMISSAO , TRB->UA_DOC , TRB->UA_SERIE , TRB->UA_FILIAL , TRB->LJ_NOME , TRB->UA_CLIENTE , TRB->UA_LOJA , TRB->A1_NOME , TRB->UA_VALBRUT , TRB->UA_VEND , Substr(TRB->A3_NOME,1,30), TRB->UA_NUM , TRB->UA_CODCON, cGRUPO , cCATEG  })
        ...
        lRegiao := .T.
        dbSelectArea("PB0")
        PB0->(dbSetOrder(1)) // Filial + Loja (Obs.: no PB0 o campo PB0_LOJA tem valor de FILIAL)
        If PB0->(dbSeek(xFilial("PB0")+TRB->UA_FILIAL))
           If mv_par07 <> "  "
              If PB0->PB0_REGVAR < mv_par07 .Or. PB0->PB0_REGVAR > mv_par08
                 lRegiao := .F.
              EndIf
           EndIf
        Else
        ...
        If SUB->(dbSeek(TRB->UA_FILIAL+TRB->UA_NUM+"01"))
           dbSelectArea("SB1")
		...
		cQuery += " AND   UA_FILIAL 	=	LJ_RPCFIL                           "
  		...
  		// Verifica apenas DOCs diferentes do Original
		cQuery += " AND   UA_TABELA 	<>	'"+cTabPgDoa+"'                    	"
		cQuery += " AND   UA_DOC     	<> 	'"+     TRB->UA_DOC     +"'        	"
		// Comentado por Mansano(08/02/06) por filtrar se forma errada os Dados
...
//				aDadosAgrega,{"A", POS->UA_EMISSAO , POS->UA_DOC , POS->UA_SERIE , POS->UA_FILIAL , POS->LJ_NOME , TRB->UA_CLIENTE , TRB->UA_LOJA , TRB->A1_NOME , POS->UA_VALBRUT , POS->UA_VEND , Substr(POS->A3_NOME,1,28), POS->UA_NUM , POS->UA_CODCON }
		...
		If (!lAchouAgreg).and.(lDataAnt==.T.)
			...
			cQuery := " SELECT MAX(UA_EMISSAO)DOACAO_ANTERIOR                 		"
			cQuery += "  FROM "+RetSqlName("SUA")+" UAZ                            	"
			cQuery += "  WHERE UA_CLIENTE   = '"+TRB->UA_CLIENTE+"'            	    "
			If MV_PAR09==1 // Considera Loja do Cliente (1=SIM e 2=NAO)
				cQuery += " AND   UA_LOJA  	= '"+TRB->UA_LOJA+"'           			"
			Endif
			cQuery += "  AND   UA_STATUS 	= 'NF.'                                 "
			cQuery += "  AND   UA_TABELA 	= '"+cTabPgDoa+"'                     	"
			...
			cQuery += "  AND   UA_EMISSAO 	< '"+DtoS(TRB->UA_EMISSAO)+"'         	"
			// LImita o periodo de Busca
			cQuery += "  AND   UA_EMISSAO 	>= '"+DtoS(dDataBase-nPrazoAnt)+"'     	"
			cQuery := ChangeQuery(cQuery)
			// Cria tabela temporária para o Relatorio
			MsAguarde({|| dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),"DOAANT",.F.,.T.)},"Aguarde...","Processando Dados...")
			// Trata campos diferente de Caracter
			For nX := 1 To Len(aStrucTRB)
				If aStrucTRB[nX][2] <> "C" .And. FieldPos(aStrucTRB[nX][1]) <> 0
					TcSetField(cAliasTRB3,aStrucTRB[nX][1],aStrucTRB[nX][2],aStrucTRB[nX][3],aStrucTRB[nX][4])
			...
			cQuery := " SELECT MAX(UA_EMISSAO)VENDA_ANTERIOR                       	"
			cQuery += " FROM "+RetSqlName("SUA")+" UAZ                             	"
			cQuery += " WHERE UA_CLIENTE   = '"+TRB->UA_CLIENTE+"'            	    "
			If MV_PAR09==1 // Considera Loja do Cliente (1=SIM e 2=NAO)
				cQuery += " AND   UA_LOJA   = '"+TRB->UA_LOJA+"'           			"
			...
			cQuery := ChangeQuery(cQuery)
			// Cria tabela temporária para o Relatorio
			MsAguarde({|| dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),"VNDANT",.F.,.T.)},"Aguarde...","Processando Dados...")
			// Trata campos diferente de Caracter
			For nX := 1 To Len(aStrucTRB)
				If aStrucTRB[nX][2] <> "C" .And. FieldPos(aStrucTRB[nX][1]) <> 0
					TcSetField(cAliasTRB3,aStrucTRB[nX][1],aStrucTRB[nX][2],aStrucTRB[nX][3],aStrucTRB[nX][4])
			...
			If !ANT->(Eof()) .and. Empty(DOAANT->DOACAO_ANTERIOR)
...
//				Aadd(aDadosAgrega,{"A" , ANT->UA_EMISSAO , ANT->UA_DOC , TRB->UA_SERIE , ANT->UA_FILIAL , ANT->LJ_NOME , TRB->UA_CLIENTE , TRB->UA_LOJA , TRB->A1_NOME , ANT->UA_VALBRUT , ANT->UA_VEND , Substr(ANT->A3_NOME,1,30) , ANT->UA_NUM, ANT->UA_CODCON})
			...
			Else
				...
				If !Empty(VNDANT->VENDA_ANTERIOR)
					If VNDANT->VENDA_ANTERIOR > DOAANT->DOACAO_ANTERIOR
        ...
        While nX <= Len(aDadosResult) .AND. lPar == .T.
            ...
            While lAchou == .T.
			   ...
			   If SUB->(dbSeek(aDadosResult[nX,1][5]+aDadosResult[nX,1][13]+_cItem))
			      _nQtd     := SUB->UB_QUANT
			...
			If Alltrim(aDadosResult[nX,1][14]) <> ""
			   dbSelectArea("PA6")
			   PA6->(dbSetOrder(1))
			   If PA6->(dbSeek(xFilial("PA6")+aDadosResult[nX,1][14]))
			...
			If Len(aDadosResult) < 2 .AND. aDadosResult[nX,1][1] = "D"
				...
				//Alterado por Regiane - 08/08/06
				cQuery := " SELECT L1.R_E_C_N_O_ AS L1RECNO, L1_EMISSAO, L1_DOC, L1_SERIE, "
				cQuery += " L1_FILIAL, L1_NOMCLI, L1_VALBRUT, L1_NUM, L1_VEND, L1_CODCON  " 
				cQuery += " FROM "+RetSqlName("SL1")+" L1                             	"
				cQuery += " WHERE L1_FILIAL		= '"+aDadosResult[nX,1][5]+"'               				"
				cQuery += " AND   L1_CLIENTE	= '"+aDadosResult[nX,1][7]+"'            	    "
				If MV_PAR09 == 1
					cQuery += " AND   L1_LOJA  	= '"+aDadosResult[nX,1][8]+"'           			"
				Endif
				If lDataAnt == .T.
					cQuery += " AND   L1_EMISSAO 	<= '"+DtoS(aDadosResult[nX,1][2])+"'         	"
					// LImita o periodo de Busca
					cQuery += " AND   L1_EMISSAO 	>= '"+DtoS(dDataBase-nPrazoAnt)+"'     	"
				Else
					cQuery += " AND   L1_EMISSAO 	= '"+DtoS(aDadosResult[nX,1][2])+"'         	"
				EndIf
				cQuery += " AND   L1_DOC <> '' "
				...
				aEval(SL1->(dbStruct()), {|z| If(z[2] # "C" .And. z[1] $ cFields, TcSetField("TMPQRY",z[1],z[2],z[3],z[4]), Nil)})
				...
				If !TMPQRY->(Eof())
					...
					//³ foi utilizada como Agregada nas vendas posteriores ³
					//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				    nPosNota := aScan(aNotaSL1,{ |x| x[1] == TMPQRY->L1RECNO })
				    If nPosNota == 0
				    	// Alimenta Array com a Nota jah utilizada como Agregada
					 	Aadd(aNotaSL1,{ TMPQRY->L1RECNO })
					 	
	                    @ Li,000 PSay "A"
				        @ Li,002 PSay DtoC(TMPQRY->L1_EMISSAO)
				        @ Li,011 PSay TMPQRY->L1_DOC
				        @ Li,017 PSay "-"
				        @ Li,018 PSay TMPQRY->L1_SERIE
				        @ Li,022 PSay TMPQRY->L1_FILIAL
				        @ Li,026 PSay Substr(TMPQRY->L1_NOMCLI,1,20)
				        @ Li,047 PSay Transform(TMPQRY->L1_VALBRUT,"999,999.99")
	                    _nItem := 1
				        _cItem := "01"
				        dbSelectArea("SL2")
				        SL2->(dbSetOrder(1)) // L2_FILIAL + L2_NUM + L2_ITEM
				        While lAchaSL2 == .T.
				        	If SL2->(dbSeek(TMPQRY->L1_FILIAL+TMPQRY->L1_NUM+_cItem))
				         ...
				         @ Li,062 PSay TMPQRY->L1_VEND
				         @ Li,069 PSay TMPQRY->L1_CODCON
				         If Alltrim(TMPQRY->L1_CODCON) <> ""
				         	dbSelectArea("PA6")
				            PA6->(dbSetOrder(1))
				            If PA6->(dbSeek(xFilial("PA6")+TMPQRY->L1_CODCON))
				            	@ Li,076 PSay Substr(PA6->PA6_DESC,1,20)
				            EndIf
	                     EndIf
	                     nTotAgrega := nTotAgrega + TMPQRY->L1_VALBRUT
