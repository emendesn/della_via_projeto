...
//³ Verifica se a forma de pagamento escolhida eh valida ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
If Upper(rTrim(ParamIxb[2][3])) == "CALCULA SINISTRO";
	.Or. Upper(rTrim(ParamIxb[2][3])) == "CONDIC. NEGOCIADA";
	.Or. Upper(rTrim(ParamIxb[2][3])) == "CONDIçãO DE PGTO";
	.Or. Upper(rTrim(ParamIxb[2][3])) == Upper(rTrim(Tabela("24", "CT")));
	.Or. Upper(rTrim(ParamIxb[2][3])) == Upper(rTrim(Tabela("24", "SG")))
	
	_lRet := .F.
	ApMsgAlert("Forma de pagamento inválida!","Não permitido")
EndIf
...
//For _n := 1 to Len(Paramixb[2])                 
	...
	If ParamIxb[2][1] <> ParamIxb[1][1]
		If ParamIxb[2][1] - dDatabase > GetMV("MV_DIASPG")
...
If (_lRet)
	...
	If SL4->(FieldPos("L4_FORMAID")) > 0
		...
		If !P_NaPilha("LJ7CONDPG");
			.And. (Upper(rTrim(ParamIxb[1][3])) == Upper(rTrim(Tabela("24", "CT")));
			.Or. Upper(rTrim(ParamIxb[1][3])) == Upper(rTrim(Tabela("24", "SG"))))
			_lRet := .F.
	...
	Else
		...
		If !P_NaPilha("LJ7CONDPG") .And. aPgtos[oPgtos:nAt][5] != Nil
			If aPgtos[oPgtos:nAt][5] $ "S/C"
				_lRet := .F.
				ApMsgAlert("Não é permitido alterar dados de parcelas de sinistro","Não permitido")
			EndIf
		EndIf
