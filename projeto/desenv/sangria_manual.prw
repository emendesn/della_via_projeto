...
do While E5_FILIAL==xFilial("SE5") .and. E5_DATA==dDataBase .and. E5_BANCO==SLF->LF_COD
	...
	elseif E5_RECPAG=="P" .and. TRIM(E5_TIPO)=="R$" .and. E5_TIPODOC=="LJ" .and. Trim(E5_NATUREZ)==&(Trim(GetMv("MV_NATDEV"))) //Troca
		...
		nTotDin-= E5_VALOR
		//			EndIf
	Elseif E5_RECPAG=="P" .and. E5_TIPODOC=="BA" .and. E5_TIPO=="NCC" // Devolucoes em especie
		nTotDin-= E5_VALOR
	EndIf
	dbSkip()
Enddo
...
_cDoca	:= ""
Do While !eof()
	// Tabula cheques recebidos via liquidacao
	If QREC->E5_MOTBX == "LIQ"
		dbSelectArea("SE1")
		dbSetOrder(15) // E1_FILIAL+E1_NUMLIQ
		dbSeek(xFilial("SE1") + AllTrim(QREC->E5_DOCUMEN))
		
		While !EOF();
			.And. SE1->E1_FILIAL == xFilial("SE1");
			.And. AllTrim(SE1->E1_NUMLIQ) == AllTrim(QREC->E5_DOCUMEN)
			...
			If dbSeek(xFilial("SEF") + SE1->(E1_PREFIXO + E1_NUM + E1_PARCELA))
				If SEF->EF_VENCTO == dDataBase
					nTotCh += SEF->EF_VALOR
				EndIf
			EndIf
