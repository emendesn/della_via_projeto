...
// Faturamento do Atacado na Loja
Private  _nFatAtac := 0
...
While !EOF()
	...
	If TRBSL1->L1_CREDITO > 0
		_CREDVENDA := _CREDVENDA + TRBSL1->L1_CREDITO
		
		SF2->(dbSetOrder(1)) // F2_FILIAL+F2_DOC+F2_SERIE+F2_CLIENTE+F2_LOJA+F2_FORMUL
		If SF2->(dbSeek(xFilial("SF2") + TRBSL1->(L1_DOC + L1_SERIE)))
			dbSelectArea("SE5")
			dbSetOrder(7) // E5_FILIAL+E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO+E5_CLIFOR+E5_LOJA+E5_SEQ
			dbSeek(xFilial("SE5") + SF2->(F2_PREFIXO + F2_DUPL))
			
			While !EOF() .And. SE5->(E5_FILIAL + E5_PREFIXO + E5_NUMERO) == xFilial("SE5") + SF2->(F2_PREFIXO + F2_DUPL)
				If SE5->E5_BANCO <> _cOperador
					dbSkip()
					Loop
				EndIf
				
				If SE5->E5_DATA <> _dDataFech
					dbSkip()
					Loop
				EndIf
				
				If SE5->E5_TIPODOC == "CP"
					cNCC := Left(SE5->E5_DOCUMEN, 3) + " " + SubStr(SE5->E5_DOCUMEN, 4, 6)
					cCli := SE5->E5_CLIFOR + " " + SE5->E5_LOJA + " " + RetField("SA1", 1, xFilial("SA1") + SE5->(E5_CLIFOR + E5_LOJA), "A1_NOME")
					aAdd(_aSCREDVENDA, {"CR", cNCC, cCli, SE5->E5_VALOR})
				EndIf
				dbSkip()
			End
		EndIf
	...
	IF EMPTY(TRBSL1->L1_SERIE) .AND. EMPTY(TRBSL1->L1_DOC)
		_cL1Serie	:= Trim(TRBSL1->L1_SERPED)
		_cL1Doc 	:= Trim(TRBSL1->L1_DOCPED)
	ELSE
		_cL1Serie	:= Trim(TRBSL1->L1_SERIE)
		_cL1Doc  	:= Trim(TRBSL1->L1_DOC)
	ENDIF
	...
	IF TRBSL1->L1_DINHEIR != 0
		If TRBSL1->L1_DINHEIR > TRBSL1->L1_VLRLIQ
...
Do While !eof()
	...
	If QREC->E5_MOTBX == "LIQ" .And. _cDoca <> QREC->E5_PREFIXO+QREC->E5_NUMERO+QREC->E5_PARCELAQ+QREC->E5_TIPO
		...
		While !EOF();
			...
			While EF_PREFIXO=_cPrefixo .and. EF_TITULO=_cTitulo .and. EF_PARCELA=_cParcela .and. EF_TIPO=_cTipo .and. !eof()
...
While !Eof() .AND. SD1->D1_FILIAL = xFilial("SD1") .AND. SD1->D1_DTDIGIT == _dEmissao
	...
	IF ALLTRIM(SD1->D1_TIPO) == "D" .And. Alltrim(_cOperador) == Alltrim(SD1->D1_NUMCQ) //se a devolucao foi feita pelo caixa
		...
		_bTemNCC := .F.
		SE1->(DbSetOrder(1))
		SE1->(DbSeek(xFilial("SE1")+"U"+cFilAnt+SD1->D1_DOC))
		While !SE1->(EOF()) .and. SE1->E1_FILIAL+SE1->E1_PREFIXO+SE1->E1_NUM == xFilial("SE1")+"U"+cFilAnt+SD1->D1_DOC .AND. !_bTemNCC
...
//		If SE5->(DbSeek(XFILIAL("SE5")+SD1->D1_SERIE+SD1->D1_DOC)) .And. SE5->E5_TIPO = "R$" .And. SE5->E5_MSFIL=SD1->D1_FILIAL
		If SE5->(DbSeek(XFILIAL("SE5")+"U"+cFilAnt+SD1->D1_DOC)) .And. SE5->E5_TIPO == "NCC" .And. SE5->E5_MSFIL == SD1->D1_FILIAL .And. SE5->E5_TIPODOC == "BA"
			//_nSangriaTotal := _nSangriaTotal + SD1->D1_TOTAL + SD1->D1_VALIPI + SD1->D1_ICMSRET - SD1->D1_VALDESC
			_nSangriaTotal += SE5->E5_VALOR
			_DINHEIRO += SE5->E5_VALOR
			_nDevolucoes := _nDevolucoes + SD1->D1_TOTAL + SD1->D1_VALIPI + SD1->D1_ICMSRET - SD1->D1_VALDESC
		Endif
		
		// Verifica se a devolu็ใo ้ NCC
		SE1->(dBSetOrder(1))
		if SE1->(DbSeek(XFILIAL("SE1")+"U"+cFilAnt+SD1->D1_DOC)) .And. SE1->E1_TIPO = "NCC" .And. SE1->E1_MSFIL=SD1->D1_FILIAL
		_nSangriaTotal += SE1->E1_VALOR
		//_nSangriaTotal := _nSangriaTotal + SD1->D1_TOTAL + SD1->D1_VALIPI + SD1->D1_ICMSRET - SD1->D1_VALDESC
		endif
...
// Verifica os titulos faturados pelo Atacado
cSql := "SELECT E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_CLIENTE,E1_LOJA,E1_NOMCLI,E1_VALOR FROM  "+RetSqlName("SE1")+" "
cSql += " WHERE D_E_L_E_T_ <> '*' "
cSql += " AND   E1_FILIAL  = '"+xFilial("SE1")+"'  "
cSql += " AND   E1_MSFIL   = '"+cFilAnt+"'    "
cSql += " AND   E1_EMISSAO  = '"+DTOS(_dEmissao)+"'     "
...
_aAtacado:={}
_nVal:=0
_cDados:=SZSE1->E1_PREFIXO+SZSE1->E1_NUM+SZSE1->E1_CLIENTE
_cRest :=SZSE1->E1_TIPO+SZSE1->E1_LOJA+SZSE1->E1_NOMCLI 

While !Eof()

        
    _nVal:=_nVal + SZSE1->E1_VALOR

	_nFatAtac := _nFatAtac + SZSE1->E1_VALOR

	dBskip()
	
	IF SZSE1->E1_PREFIXO+SZSE1->E1_NUM+SZSE1->E1_CLIENTE<>_cDados
   	    AADD(_aAtacado,{Substr(_cDados,1,3),Substr(_cDados,4,6)," ",substr(_cRest,1,3),Substr(_cRest,4,2),Substr(_cRest,6,20),_nVal})
       _cDados:=SZSE1->E1_PREFIXO+SZSE1->E1_NUM+SZSE1->E1_CLIENTE	
       _cRest :=SZSE1->E1_TIPO+SZSE1->E1_LOJA+SZSE1->E1_NOMCLI        
       _nVal  :=0
	Endif 
...
While !Eof() .AND. AE_FILIAL == xFilial("SAE")
	Do Case
		...
		Case AE_TIPO == "SG" .Or. AE_TIPO == "CT"
			//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
			//ณ Corretora e Seguradora sao tipos financiados ณ
...
/*
@ _nLin,000 PSAY "|"
_cTexto := " *** Faturamento pelo Atacado "
@ _nLin,002 PSAY _cTexto+Replicate("-",60-Len(_cTexto))+">"
@ _nLin,065 PSAY _nFatAtac PICTURE "@E 99,999,999.99"
@ _nLin,079 PSAY "|"
AddLinha()
...
// Inicio Impressao das Duplicatas do Atacado
_cTexto := "Faturamento pelo Atacado"
@ _nLin,000 PSAY "|"
@ _nLin,002 PSAY _cTexto
@ _nLin,079 PSAY "|"
...
aSort(_aAtacado,,,{|x,y| x[5]+x[1]+x[2]+x[3] > y[5]+y[1]+y[2]+y[3]})
...
If _cTipoRel <> 1
	...
	For _nCont:=1 to Len(_aAtacado)
		@ _nLin,000 PSAY "|"
		@ _nLin,003 PSAY _aAtacado[_nCont][1]					//Prefixo
		@ _nLin,007 psay _aAtacado[_nCont][2]					//Titulo
		@ _nLin,014 psay _aAtacado[_nCont][3]                   //Parcela
		@ _nLIn,017 psay _aAtacado[_nCont][4]                   //Tipo
		@ _nLin,022 psay _aAtacado[_nCont][5]                   //Cliente
		@ _nLin,030 psay _aAtacado[_nCont][6] PICTURE "@S30"    //Nome do Clieente
		@ _nLin,065 psay _aAtacado[_nCont][7] PICTURE "@E 99,999,999.99"  //Valor
		@ _nLin,079 PSAY "|"
		_nTotRec2 := _nTotRec2 + _aAtacado[_nCont][7]
		AddLinha()
	Next
	@ _nLin,000 PSAY "|"
	@ _nLin,002 PSAY _cTexto+" (TOTAL) "+Replicate("-",60-(Len(_cTexto)+9))+">"
	@ _nLin,065 PSAY _nTotRec2 PICTURE "@E 99,999,999.99"
	@ _nLin,079 PSAY "|"
	AddLinha()
	@ _nLin,000 PSAY "|"
	@ _nLin,079 PSAY "|"
...
Else
	...
	AddLinha()
Endif
//  Fim Impressao das Duplicatas do Atacado

@ _nLin,000 PSAY "|"
_cTexto := "Total das Duplicatas / NF "
@ _nLin,002 PSAY _cTexto+Replicate("-",60-Len(_cTexto))+">"
@ _nLin,065 PSAY _nOutros+_nFatAtac  PICTURE "@E 99,999,999.99"
@ _nLin,079 PSAY "|"
AddLinha()

@ _nLin,000 PSAY "|"
@ _nLin,001 PSAY Replicate ('-',078)
@ _nLin,079 PSAY "|"
AddLinha()
...
// Inicio Alteracao gsabino em 10.12.2007  (Projeto do Caixa Central - Imprimir dados do Banco Padrao da Loja para Efeito de Deposito)'
@ _nLin,000 PSAY "|"
@ _nLin,002 PSAY " *** Banco para Deposito ---> : "+Posicione("SLJ",1,xFilial("SLJ")+PADL(SM0->M0_CODFIL,6,"0"),"LJ_BCODEP")+" - Agencia "+Posicione("SLJ",1,xFilial("SLJ")+PADL(SM0->M0_CODFIL,6,"0"),"LJ_AGDEP")+" - Conta "+Posicione("SLJ",1,xFilial("SLJ")+PADL(SM0->M0_CODFIL,6,"0"),"LJ_CNTDEP")
@ _nLin,079 PSAY "|"
AddLinha()
// Fim    Alteracao gsabino em 10.12.2007  (Projeto do Caixa Central - Imprimir dados do Banco Padrao da Loja para Efeito de Deposito)'
...
Static Function ImprimRec(_dDataFech)
aAreaAtu:=GetArea()
...
titulo         := "PROTOCOLO  DE  RECEBIMENTO "

_cQuery := "SELECT * FROM "+RetSqlname("SE5")+" WHERE E5_FILIAL = '"+xFilial("SE5")+"' AND D_E_L_E_T_<>'*' AND E5_MSFIL = '"+MV_PAR01+"' "
_cQuery += " AND E5_DATA = '"+DTOS(MV_PAR02)+"'  "
_cQuery += " AND E5_NATUREZ = 'SANGRIA'   "
_cQuery += " AND E5_RECPAG  = 'R'  "

IF MV_PAR03 = 1   // R$
	_cQuery += " AND E5_MOEDA   = 'R$'        "
ENDIF

IF MV_PAR03 = 2  // CH a VISTA
	_cQuery += " AND E5_MOEDA = 'CH'  AND SUBSTR(E5_AGENCIA,1,1)<>'' AND SUBSTR(E5_NUMERO,1,1)=''  "
ENDIF

IF MV_PAR03 = 3  // R$ e CH a VISTA
	_cQuery += " AND ((E5_MOEDA   = 'R$'   OR   E5_MOEDA = 'CH')  AND SUBSTR(E5_AGENCIA,1,1)<>'' AND SUBSTR(E5_NUMERO,1,1)='')  "
ENDIF
...
dBsetOrder(1)
dbseek(xFilial("SLJ")+PADL(MV_PAR01,6,"0"))
_lOk1:=.F.
IF ALLTRIM(LJ_XBCODEP) $ GETMV("ES_BCOMAT")
	_lOk1:=.T.
ENDIF

_lOk2:=.T.
IF ALLTRIM(LJ_XAGDEP) $ GETMV("ES_BCOMAT")
	_lOk2:=.T.
ENDIF

_lOk3:=.T.
IF ALLTRIM(LJ_XCNTDEP) $ GETMV("ES_BCOMAT")
	_lOk3:=.T.
ENDIF

IF _lOk1 .and. _lOk2 .and. _lOk3
	_cQuery += " AND E5_BANCO   = '"+SLJ->LJ_XBCODEP+"'  "
	_cQuery += " AND E5_AGENCIA = '"+SLJ->LJ_XAGDEP+"'  "
	_cQuery += " AND E5_CONTA   = '"+SLJ->LJ_XCNTDEP+"'  "
Endif

/*
_cQuery += " AND RTRIM(E5_BANCO)   LIKE '%"+GETMV("ES_BCOMAT")+"%'   "
_cQuery += " AND RTRIM(E5_AGENCIA) LIKE '%"+GETMV("ES_BCOMAT")+"%'   "
_cQuery += " AND RTRIM(E5_CONTA)   LIKE '%"+GETMV("ES_BCOMAT")+"%'   "
*/
IF SELECT("SZSE5") <> 0
	dBselectarea("SZSE5")
	dBClosearea()
...
TCQUERY _cQuery New Alias "SZSE5"
TcSetField("SZSE5","E5_DATA"  ,"D",08,0)
...
While ! Eof()
	...
	IF !(E5_BANCO = SLJ->LJ_XBCODEP  .AND. E5_AGENCIA = SLJ->LJ_XAGDEP  .AND. E5_CONTA   = SLJ->LJ_XCNTDEP)
		dBselectarea("SZSE5")
		dBskip()
		loop
	ENDIF
...
IF  SZSE5->E5_MOEDA = "R$"
	@ li,028 PSAY "***   D  I  N  H  E  I  R  O  *** "
ELSEIF SZSE5->E5_MOEDA = "CH"
	@ li,020 PSAY "***   C  H  E  Q  U  E    A    V I S T A  *** "
ENDIF
li++
li++
li++
@ li,000 PSAY "BCO:     "+SZSE5->E5_BANCO
@ li,014 PSAY "AGENCIA: "+SZSE5->E5_AGENCIA
@ li,030 PSAY "CONTA:   "+SZSE5->E5_CONTA
@ li,062 PSAY "R$"
@ li,066 PSAY SZSE5->E5_VALOR             PICTURE "@ZE 999,999,999.99"
li++
li++

@ li,001 PSAY "RECEBI DA LOJA "+SZSE5->E5_MSFIL+"-"+Posicione("SLJ",1,xFilial("SLJ")+PADL(SZSE5->E5_MSFIL,6,"0"),"LJ_NOME")
@ li,054 PSAY "O VALOR DE  "+TRANS(SZSE5->E5_VALOR,"@R 999,999,999.99")
li++
li++
@ Li,000 PSAY ALLTRIM(EXTENSO(SZSE5->E5_VALOR)) + Replicate('*',80-Len(Alltrim(EXTENSO(SZSE5->E5_VALOR))))
li++
li++
li++
@ li,000 PSAY " RELATIVO A SANGRIA NO FECHAMENTO DO CAIXA DA DATA DE "+DTOC(SZSE5->E5_DATA)
li++
li++
li++
@ li,000 PSAY " OBS: SANGRIA REFERENTE A "+SZSE5->E5_MOEDA
li++
li++
li+=3
@ li,000 PSAY "-------------------------"
...
li+=1
@ li,000 PSAY "         L O J A         "
@ li,026 PSAY "      G E R E N T E      "
@ li,052 PSAY "     CONTAS A PAGAR      "
li+=2
