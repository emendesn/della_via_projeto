...
While TRB->(!Eof())
	...
	If !Empty(TRB->PRODPAI)
		...
		Subs(TRB->PRODPAI,06,04),;
		Subs(TRB->PRODPAI,11,06),;
		Subs(TRB->PRODPAI,16,15),;
		TRB->L2_QUANT,;
		TRB->B1_PRV1,;
...
For nX := 1 to Len(aDados)
	...
	If cOldCateg <> aDados[nX,1]
		...
		If !Empty(cOldCateg)
			...
			Li++
			Li++
			@ Li,000 PSay "TOTAL DA CATEGORIA"
