...
Private cPerg    	:= "DVR002"
Private nNomeProg 	:= "DVLOJR02"
Private cString  	:= "SUA"
...
Local cAliasTRB 	:= "TRB" // Alias para vendas de Doacao por periodo - UA
Local cAliasTRB2 	:= "POS" // Alias para vendas Posteriores a DOACAO  - UA
Local cAliasTRB3 	:= "ANT" // Alias para vendas Anteriores  a DOACAO  - UA

Local cAliasTL1 	:= "TL1" // Alias para vendas de Doacao por periodo - L1
Local cAliasTL12 	:= "PL1" // Alias para vendas Posteriores a DOACAO  - L1
Local cAliasTL13 	:= "AL1" // Alias para vendas Anteriores  a DOACAO  - L1
Local nX			:= 0      
Local lAchouAgreg 	:= .F.
Local lAchouAgL1  	:= .F.
Local aStrucTRB		:= SUA->(dbStruct())
Local aStrucTL1		:= SL1->(dbStruct())
Local nTotDoa   	:= 0 
Local nTotAgrega	:= 0 
Local lDataAnt		:= GetMV("FS_DEL011")  // Pesquisa Notas Agregadas anteriores a Doacao
Local cTabPgDoa	    := GetMV("FS_DEL048")  // Tabela de precos para DOACAO - Call Center
Local cCondPgDoa    := GetMV("FS_DEL012")  // Condição de Pagto - Sigaloja
Local aDadosDoa		:= {}
Local aDadosAgrega	:= {}
Local aDadosResult	:= {}
Local aDadosDL1		:= {}
...
Local aNFL1			:= {}
Local Cabec2		:= ""          
Local Cabec1		:= "T DT.EMISS NF-SERIE. DESCRICAO                   COD CLI   NOME CLIENTE                     VL TOTAL VENDEDOR                      "
...
cQuery += " ORDER BY L1_CLIENTE,L1_LOJA,L1_EMISSAO,L1_DOC,L1_SERIE    				"
cQuery := ChangeQuery(cQuery)

// Cria tabela temporaria para o Relatorio
MsAguarde({|| dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasTL1,.T.,.T.)},"Aguarde...","Processando Dados...")

// Trata campos diferente de Caracter
For nX := 1 To Len(aStrucTL1)
	If aStrucTL1[nX][2] <> "C" .And. FieldPos(aStrucTL1[nX][1]) <> 0
		TcSetField(cAliasTL1,aStrucTL1[nX][1],aStrucTL1[nX][2],aStrucTL1[nX][3],aStrucTL1[nX][4])
	EndIf
Next nX
//-Alteração para inclusão do SL1 - termino
...
While TRB->(!Eof()) 
	IncRegua()
		...
		Aadd(aDadosDoa,{"D" , TRB->UA_EMISSAO , TRB->UA_DOC , TRB->UA_SERIE , TRB->UA_FILIAL , TRB->LJ_NOME , TRB->UA_CLIENTE , TRB->UA_LOJA , TRB->A1_NOME , TRB->UA_VALBRUT , TRB->UA_VEND,Substr(TRB->A3_NOME,1,30),TRB->UA_NUM})
...
//  SL1 - TL1
//
//- Alteração para processamento do SL1
While TL1->(!Eof()) 
	IncRegua()
		If Li > 60
			Cabec(cTitulo,cabec1,cabec2,nNomeprog,cTamanho,nTipo)
		...
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		aDadosDL1 := {}
		Aadd(aDadosDL1,{"D",TL1->L1_EMISSAO,TL1->L1_DOC,TL1->L1_SERIE,TL1->L1_FILIAL,;
		                TL1->LJ_NOME,TL1->L1_CLIENTE,TL1->L1_LOJA,TL1->A1_NOME,TL1->UA_VALMERC,;
						TL1->L1_VEND,Substr(TL1->A3_NOME,1,30)})
		...
		cQuery := "SELECT L1_CONDPG,L1_EMISSAO,L1_DOC,L1_SERIE,				    "
		cQuery += " L1_FILIAL,LJ_NOME,                                         	"
		cQuery += " L1_VALMERC,L1_VEND,A3_NOME,L1.R_E_C_N_O_                   	"
		cQuery += " FROM " + RetSqlName("SL1")+" L1,                           	"
		cQuery +=            RetSqlName("SA3")+" A3, "+RetSqlName("SLJ")+" LJ 	"
		cQuery += " WHERE L1_CLIENTE       = '"+TL1->L1_CLIENTE+"'             	"

		// Considera Loja do Cliente (1=SIM e 2=NAO) 
		If MV_PAR09==1
			cQuery += " AND   L1_LOJA          = '"+TL1->L1_LOJA+"'           	"
		Endif
		cQuery += " AND   L1_CONDPG 	<>	'"+cCondPgDoa+"'                    "
		cQuery += " AND   L1_EMISSAO 	>= 	'"+DtoS(TL1->L1_EMISSAO)+"'        	"
		cQuery += " AND   L1_DOC     	<> 	'"+     TL1->L1_DOC     +"'        	"
		cQuery += " AND   L1_SERIE   	<> 	'"+     TL1->L1_SERIE   +"'        	"
		cQuery += " AND   L1.D_E_L_E_T_ <> 	'*'                                 "
		cQuery += " AND   A3.D_E_L_E_T_ <> 	'*'                                 "
		cQuery += " AND   LJ.D_E_L_E_T_ <> 	'*'                                 "
		cQuery += " AND   A3_FILIAL 	= 	'"+xFilial("SA3")+"'               	"
		...
		cQuery += " AND   L1_VEND   	=	A3_COD                              "
		cQuery += " AND   L1_FILIAL 	=	LJ_RPCFIL                           "
  		cQuery += " AND   L1_STATUS 	= 	'NF.'                               "
		cQuery += " ORDER BY L1_EMISSAO ASC                                    	"
		cQuery += " FETCH FIRST 1 ROWS ONLY                                    	"
		cQuery := ChangeQuery(cQuery)

		// Cria tabela temporaria para o Relatorio
		MsAguarde({|| dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasTL12,.F.,.T.)},"Aguarde...","Processando Dados...")

		// Trata campos diferente de Caracter
		For nX := 1 To Len(aStrucTRB)
			If aStrucTL1[nX][2] <> "C" .And. FieldPos(aStrucTL1[nX][1]) <> 0
				TcSetField(cAliasTL12,aStrucTL1[nX][1],aStrucTL1[nX][2],aStrucTRB[nX][3],aStrucTL1[nX][4])
			EndIf
		Next nX
		...
		If !PL1->(Eof())
			...
			//³ foi utilizada como Agregada nas vendas posteriores ³
			//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		    nPosNota := aScan(aNFL1,{ |x| x[1] == PL1->R_E_C_N_O_ })
		    If nPosNota == 0                      
		    	...
		    	// Alimenta Array com a Nota jah utilizada como Agregada
				Aadd(aNFL1,{PL1->R_E_C_N_O_})

				// Alimenta Arrays com os dados da Venda Agregada para Impressao posterior
				aDadosAgrega := {}

				Aadd(aDadosAgrega,{"A",PL1->L1_EMISSAO,PL1->L1_DOC,PL1->L1_SERIE,PL1->L1_FILIAL,;
				                   PL1->LJ_NOME,TL1->L1_CLIENTE,TL1->L1_LOJA,TL1->A1_NOME,;
								   PL1->L1_VALBRUT,PL1->L1_VEND,Substr(PL1->A3_NOME,1,28)})
		    Endif
		Endif
		...
		//³ lDataAnt=.F. ignora vendas anteriores.             ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		If (!lAchouAgL1).and.(lDataAnt==.T.)
			cQuery := "SELECT L1_CONDPG,L1_EMISSAO,L1_DOC,L1_SERIE,    				"
			cQuery += " L1_FILIAL,LJ_NOME,                                         	"
			cQuery += " L1_VALBRUT,L1_VEND,A3_NOME,                                 	"
...
//			cQuery += " (SELECT UA_DOC                                     			"
			...
			// Cria tabela temporária para o Relatorio
			MsAguarde({|| dbUseArea(.T.,"TOPCONN",TcGenQry(,,cQuery),cAliasTL13,.F.,.T.)},"Aguarde...","Processando Dados...")

			// Trata campos diferente de Caracter
			For nX := 1 To Len(aStrucTL1)
				If aStrucTL1[nX][2] <> "C" .And. FieldPos(aStrucTL1[nX][1]) <> 0
					TcSetField(cAliasTL13,aStrucTL1[nX][1],aStrucTL1[nX][2],aStrucTL1[nX][3],aStrucTL1[nX][4])
				EndIf
			Next nX               
			
			// Pega o Primeiro Registro da Sub-Query de Venda Anterior a DOACAO 
			If !AL1->(Eof()) .and. Empty(AL1->DOACAO_ANTERIOR)

				// Informa que ja achou a Venda Agregada
				lAchouAgL1  := .T.
				aDadosAL1    := {}
				Aadd(aDadosAL1   ,{"A",AL1->L1_EMISSAO,AL1->L1_DOC,TL1->L1_SERIE,AL1->L1_FILIAL,;
				                   AL1->LJ_NOME,TL1->L1_CLIENTE,TL1->L1_LOJA,TL1->A1_NOME,;
								   AL1->L1_VALMERC,AL1->L1_VEND,Substr(AL1->A3_NOME,1,30)})
			Else                             
				...
				//³ quer dizer que a Doacao da Sub-Query jah possui um venda agregada            ³
				//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
				If !Empty(AL1->VENDA_ANTERIOR)
					If AL1->VENDA_ANTERIOR > AL1->DOACAO_ANTERIOR
		...
		// Se localizou Venda Agregada
		If Len(aDadosAL1   ) > 0
			If aDadosDL1[1,2] <= aDadosAL1   [1,2] // Compara EMISSAO
				Aadd(aDadosRL1,aDadosDL1)
				Aadd(aDadosRL1,aDadosAL1)
			Else
				Aadd(aDadosRL1,aDadosAL1)
				Aadd(aDadosRL1,aDadosDL1)
			Endif                                    
		...
		//³ Imprime as Vendas de Doacoes e Agragacoes ³
		//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
		For nX := 1 to Len(aDadosRL1)

			@ Li,000 PSay aDadosRL1[nX,1][1]  // D=Doacao / A=Agregacao
			@ Li,002 PSay aDadosRL1[nX,1][2]  // Emissao
			@ Li,011 PSay aDadosRL1[nX,1][3]  // Nr. Atendimento
			@ Li,018 PSay aDadosRL1[nX,1][4]  // Loja
			@ Li,021 PSay aDadosRL1[nX,1][5]  // Descricao da Loja
			@ Li,046 PSay aDadosRL1[nX,1][6]  // Codigo Cliente
			@ Li,053 PSay aDadosRL1[nX,1][7]  // Loja Cliente
			@ Li,056 PSay Substr(aDadosRL1[nX,1][8],1,30)  // Nome Cliente
			@ Li,087 PSay Transform((aDadosRL1[nX,1][9]),"999,999.99")  // Total da Operacao
			@ Li,098 PSay aDadosRL1[nX,1][10]                        // Codigo Vendedor
			@ Li,105 PSay Substr(aDadosRL1[nX,1][11],1,28)          // Nome Vendedor
			Li++
            
            // - Totaliza por Doacao e Agregado
//          If  aDadosRL1[nX,1][1] == 'A'
//             nTotAgrega := nTotAgrega + aDadosRL1[nX,1][9]
//          ElseIf aDadosRL1[nX,1][1] == 'D'
//             nTotDoa :=nTotDoa + aDadosRL1[nX,1][9]
//          EndIf   
		Next i 
		Li++
