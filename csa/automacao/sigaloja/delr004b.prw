...
cQuery := "select ("
cQuery += "select K1.B1_SPECIE2 || '-' || rtrim(K2.L2_PRODUTO) || '-' || K2.L2_DESCRI "
...
While TRB->(!Eof())
	...
	If !Empty(TRB->PRODPAI)
		Aadd(aDados,{;
		Subs(TRB->PRODPAI,01,04),;
		Subs(TRB->PRODPAI,06,06),;
		Subs(TRB->PRODPAI,13,15),;
		TRB->L2_QUANT,;
		TRB->B1_PRV1,;
...
For nX := 1 to Len(aDados)
	...
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	If (!Empty(cOldProduto)) .and. (cOldProduto <> aDados[nX,2])
		@ Li,002 PSay Iif(nX=1,aDados[nX,2],cOldProduto)					// Codigo
		@ Li,009 PSay aDados[nX-Iif(nX>1,1,0),3]  						// Descricao
		@ Li,028 PSay nTotProQtd      	 			Picture "@E 999999"    	// Quantidade
		@ Li,035 PSay "*"
		@ Li,038 PSay aDados[nX-Iif(nX>1,1,0),5] 	Picture "@E 999,999.99"	// Vlr.Unitario
	...
	If cOldEspecie <> aDados[nX,1]
		...
		If !Empty(cOldEspecie)
			...
			cOldEsCabec := aDados[nX,1]
			
		Endif
		SZ7->(dbSeek(xFilial("SZ7") + Iif(Empty(cOldEsCabec),aDados[nX,1],cOldEsCabec)))
		
		// Imprime Sub Dicisao por Especie
		@ Li,000 PSay Iif(Empty(cOldEsCabec),aDados[nX,1],cOldEsCabec)
		@ Li,005 PSay rTrim(SZ7->Z7_DESC)
	...
	// Totalizadores
	nTotProQtd 	+= aDados[nX,4] // Quantidade PRODUTOS
	nTotProVlr 	+= aDados[nX,6] // Vlr.Total
	nTotEspQtd 	+= aDados[nX,4] // Quantidade ESPECIES
	nTotEspVlr 	+= aDados[nX,6] // Vlr.Total
	nTotGerQtd 	+= aDados[nX,4] // Quantidade GERAL
	nTotGerVlr 	+= aDados[nX,6] // Vlr.Total
	// Variaveis de Acumulo
	cOldEspecie := aDados[nX,1] // Especie
	cOldProduto := aDados[nX,2] // Codigo
...
If Len(aDados) > 0
	...
	@ Li,009 PSay aDados[Len(aDados),3]  	// Descricao
	@ Li,028 PSay nTotProQtd 		Picture "@E 999999"    	// Quantidade
	@ Li,035 PSay "*"
	@ Li,038 PSay aDados[Len(aDados),5] 	Picture "@E 999,999.99"	// Vlr.Unitario
	@ Li,058 PSay nTotProVlr      	Picture "@E 999,999.99"	// Vlr.Total
	Li++
	Li++
	@ Li,002 PSay "TOTAL DA ESPECIE"
