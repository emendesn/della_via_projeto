...
±±ºPrograma  ³sangriaA  ºAutor  ³Marcelo Alcantara / Marcio Domingos      º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³esta Rotina efetua o Fechamento do Caixa com o emissão do   º±±
±±º          ³relatorio de fechamento analitica e Sintetico               º±±                                             º±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºFun‡„o    ³RUNREPORT º Autor ³ AP5 IDE            º Data ³  01/06/00   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
While !EOF()
	...
	//verifica se a nf/cupom é um credito de venda
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³RELFECX   ºAutor  ³                    º Data ³  10/22/04   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
Static Function ImpCab() // Funcao para impressao do cabecalho
nPag += 1
If _nLin > 53
	@ _nLin,000 PSAY "|"
	@ _nLin,001 PSAY Replicate("-",limite)
...
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÿ¸Qó¿
//³Funcao para Fechamento do Caixa³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÿ$[ðÙ
...
if nSaldoCx >= 0 .and. nSaldoCx <= nFundoCx  //se o saldo estiver dentro do fundo de caixa pocibilita fechar o caixa
	if Aviso("Fechamento do Caixa","O Saldo da Loja em "+DtoC(_dEmissao)+" é "+Transform(nSaldoCx, "@E 9,999,999.99")+;
		"             Deseja Fecha-lo Agora??",{"Sim","Não"}) == 2
		if Aviso("Relatorio","Imprimir Relatorio para conferencia?",{"Sim","Não"}) == 1
			//			sair()
			Return 2
		else
			sair()
	...
	if Aviso("Fechamento do Caixa","O Saldo da Loja em "+DtoC(_dEmissao)+" é "+Transform(nSaldoCx, "@E 999,999.99")+;
		"  Não é possível Fechar o Caixa, pois Saldo esta Inconsistente! Deseja imprimir relatorio apenas para conferencia??",{"Sim","Não"},2) == 2
		sair()
		Return 0
	else
		Return 2
...
//Funcao para pegar o numero de serie do financeiro SE1
