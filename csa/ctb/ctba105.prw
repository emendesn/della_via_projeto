...
While (cAliasCTK)->(Ctb105Grv(	cSeqChave,lAglut,aCampos,dDataLanc,@cSubLote,@nLin,;
											@nRecCTK, cAliasCTK, @aFlagCTB, @aCTKxCT2 ))
	...
	If !lDigita										// Se altera lancamento -> visualiza!!
		If lRpc
			...
			CTBGrava(3,dDatalanc,cLote,cSubLote,cDoc,lAglut,cSeqChave,;
							__lCusto,__lItem,__lCLVL,,cProgName,cPreLcto,dReproc,cEmpOri,cFilOri,@aFlagCTB,@aCTKxCT2)					
	...
	If lDigita
		...
		While nOpca == 0
			...
			If Type("oMainWnd") = "U"
				...
				dReproc,cEmpOri,cFilOri,@aFlagCTB,@aCTKxCT2)
		   ...
		   @ 21 ,65 	MSGET oInf 	VAR nTotInf 	Picture "@E 999,999,999,999.99" Of oPanel3 READONLY SIZE 95 ,9 PIXEL
		   @ 41 ,65 	MSGET oDig 	VAR aTotRdpe[1][1]	Picture "@E 999,999,999,999.99" Of oPanel3 READONLY SIZE 95 ,9 PIXEL
		   ...
		   @ 21 ,240	MSGET oDeb 	VAR aTotRdPe[1][2]	Picture "@E 999,999,999,999.99" Of oPanel3 READONLY SIZE 95 ,9 PIXEL
		   @ 41 ,240	MSGET oCred 	VAR aTotRdPe[1][3] Picture "@E 999,999,999,999.99" Of oPanel3 READONLY SIZE 95 ,9 PIXEL

			
			ACTIVATE MSDIALOG oDlg ON INIT (oGetDB:oBrowse:Refresh(),EnchoiceBar(oDlg,{||nOpca:=1,If(	Ct105TOK(lCT105TOK,lCT105CHK,oGetDB:lModified,,aTotRdpe,nTotInf) .And.	Ctb101Doc(dDataLanc,cLote,cSubLote,@cDoc,oDoc,CTF_LOCK,nOpc) .And.	Ct102GrCTF(dDataLanc,cLote,cSubLote,cDoc,@CTF_LOCK),	oDlg:End(), nOpca := 0)}, {||oDlg:End()},,aButton) )
		...
		IF nOpcA == 1
			...
			EndIf					
			CTBGrava(3,dDatalanc,cLote,cSubLote,cDoc,lAglut,cSeqChave,;
			__lCusto,__lItem,__lCLVL, nTotInf,cProgName,cPreLcto,dReproc,cEmpOri,cFilOri,@aFlagCTB,@aCTKxCT2)
...
If dReproc <> Nil
	...
	If CT2->CT2_FILIAL = xFilial("CT2") .And. CT2->CT2_DATA > dReproc
		CTBA190(.T.,dReproc,CT2->CT2_DATA,cFilAnt,cFilAnt,"1",.F.,"  ")
	Else                    	
		CTBA190(.T.,dReproc,dReproc,cFilAnt,cFilAnt,"1",.F.,"  ")
	Endif
Endif
...
Return .T.
	...
	While !Eof() .and. (cAliasCTK)->CTK_SEQUEN == cSeqChave .And. nLinha < MAX_LINHA[1]
		...
		If !lZero .Or. ((cAliasCTK)->CTK_DC == "4" .And. ! lAglut)
			...
			/// Verifica o array aFlagCTb ( Marcacao dos flags de contabilização debtro da transação )
...
//					nAFlag := nPosAFlag			/// POSICAO INICIAL DO ARRAY CORRESPONDENTE AO LANCAMENTO
					For nAFlag := 1 to Len(aFlagCTB)
						If aFlagCTB[nAFlag][6] == (cAliasCTK)->(Recno())
...
//				EndIf
				...
				If CT5->(MsSeek(cKeyCT5)) 		/// SE ACHAR O LANCAMENTO PADRAO DE ORIGEM
					If !Empty(CT5->CT5_HAGLUT)  /// E O HISTORICO AGLUTINADO ESTIVER PREENCHIDO
						#IFDEF TOP
							...
							Else
						#ENDIF
							If lSomaLinha					/// SE NAO 1º REGISTRO AGLUTINACAO CODEBASE (TOP SEMPRE .T.)
								If lCTKHAGLUT
									lGravaHAglut := .F.
									lGravaHist	 := .T.
								Else
	...
	ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
	/*/
	Function CT105LINOK(cCampo,lEfetiva,aErro)
	...
	If lRet	                                                                       
		...
		dbGoTop()
		While TMP->(!Eof())	
			If !CT105LINOK()	// Valida a Linha do Lan‡amento pela CT105LinOk()
				lRet	:= .F.
				Exit
			Endif			
			dbSkip()
