...
/*		
		IF SF2->F2_SERIE <> mv_par03    // Se a Serie do Arquivo for Diferente
			MsgBox("Nao existem notas a serem impressas. Verifique os parametros!","Atencao","INFO")
...
*/		
		...
		While !eof() .and. SD2->D2_DOC== xNUM_NF .and. SD2->D2_SERIE == xSERIE
            ...
            _cDesc := Alltrim(SC2->C2_PRODUTO)+" "+Alltrim(SC2->C2_X_DESEN)+" "+IIF(Empty(SC2->C2_NUMFOGO),SC2->C2_SERIEPN,"NF-"+SC2->C2_NUMFOGO)
			...
			IF (SB1->B1_TIPO == "MO" .and. Alltrim(SB1->B1_GRUPO) $ _cGrpRO ) .or. (SB1->B1_TIPO == "MO" .and. Alltrim(SB1->B1_GRUPO) == "SERV") .or. ;
				...
				IF SC2->C2_X_STATU != "9"
					aAdd(aCOD_SERV,{SC6->C6_ITEMOP,SB1->B1_GRUPO,SD2->D2_QUANT,SD2->D2_UM,SB1->B1_DESC,SD2->D2_PRCVEN,SD2->D2_TOTAL,SD2->D2_TES,SD2->D2_PEDIDO,SD2->D2_ITEMPV,_cOP})
				Else
					aAdd(aCOD_SERV,{"99",SB1->B1_GRUPO,SC2->C2_QUANT,"RECUSADO",_cDesc,SD2->D2_PRCVEN,SD2->D2_TOTAL,SD2->D2_TES,SD2->D2_PEDIDO,SD2->D2_ITEMPV})
				EndIF	
				//aAdd(aCOD_SERV,{SD2->D2_QUANT,SD2->D2_UM,SB1->B1_DESC,SD2->D2_PRCVEN,SD2->D2_TOTAL,SD2->D2_TES,SD2->D2_PEDIDO,SD2->D2_ITEMPV,SB1->B1_GRUPO})				
			Else				
				aAdd(aCOD_PROD,{SD2->D2_COD,SB1->B1_DESC,SB1->B1_CLASFIS,SD2->D2_CLASFIS,SD2->D2_UM,SD2->D2_PRCVEN,SD2->D2_TOTAL,IIF(Empty(SD2->D2_PICM),0,SD2->D2_PICM),SD2->D2_TES,_cOP,SD2->D2_PEDIDO,SD2->D2_ITEMPV,SB1->B1_GRUPO})
			EndIF
...
nLin      := 3
@ nLin, 000 PSAY CHR(27)+"0" //1/8
//@ nLin, 000 PSAY Chr(15)                // Compressao de Impressao

@ nLin, 156 PSAY xNUM_NF                // Numero da Nota Fiscal
...
@ nLin, 032 PSAY xNATUREZA               // Texto da Natureza de Operacao

If mv_par04 == 1
	@ nLin, 073 PSAY xCF[1] Picture PESQPICT("SD1","D1_CF") // Codigo da Natureza de Operacao
Else
	@ nLin, 072 PSAY xCF[1] Picture PESQPICT("SD2","D2_CF") // Codigo da Natureza de Operacao
EndIf
...
@ nLin, 032 PSAY xNOME_CLI              //Nome do Cliente
@ nLin, 074 PSAY xCOD_CLI              //Nome do Cliente

IF !EMPTY(xCGC_CLI)                   // Se o C.G.C. do Cli/Forn nao for Vazio
	@ nLin, 116 PSAY xCGC_CLI Picture "@R 99.999.999/9999-99"
Else
	@ nLin, 110 PSAY " "                // Caso seja vazio
Endif

@ nLin, 151 PSAY xEMISSAO              // Data da Emissao do Documento

nLin+=1

@ nLin, 032 PSAY xEND_CLI                                 // Endereco
@ nLin, 111 PSAY xBAIRRO                                  // Bairro
@ nLin, 127 PSAY xCEP_CLI Picture"@R 99999-999"           // CEP
@ nLin, 143 PSAY " "                                      // Reservado  p/Data Saida/Entrada

nLin+=2

@ nLin, 032 PSAY xMUN_CLI                               // Municipio
@ nLin, 081 PSAY xTEL_CLI                               // Telefone/FAX
@ nLin, 109 PSAY xEST_CLI                               // U.F.
@ nLin, 117 PSAY xINSC_CLI                              // Insc. Estadual
@ nLin, 137 PSAY " "                                    // Reservado p/Hora da Saida
...
IF mv_par04 == 2
	IF nToTDesc > 0
		nLin:= 39
		@ nLin, 41 Psay "(-)Desconto Ref.Conserto: "
		@ nLin, 67 Psay nToTDesc Picture "@E@Z 999,999,999.99"
	EndIF
	IF !Empty(xMENSAGEM)
		nLin:=40
	...
	IF!Empty(xCOD_MENS)
		nLin:=41
		@ nLin, 41 PSAY FORMULA(xCOD_MENS) // Imprime Mensagem Padrao da Nota Fiscal
	Endif
EndIF
...
If xVALISS > 0
	@ 41, 120  PSAY xVALISS  Picture "@E@Z 999,999,999.99"   // Valor do ISS
	@ 41, 146  PSAY xTOT_FAT  Picture "@E@Z 999,999,999.99"   // Valor do Servico
Endif
//+-------------------------------------+
//¦ Calculo dos Impostos                ¦
//+-------------------------------------+

@ 43, 040  PSAY xBASE_ICMS  Picture "@E@Z 999,999,999.99"  // Base do ICMS
@ 43, 065  PSAY xVALOR_ICMS Picture "@E@Z 999,999,999.99"  // Valor do ICMS
@ 43, 090  PSAY xBSICMRET   Picture "@E@Z 999,999,999.99"  // Base ICMS Ret.
@ 43, 118  PSAY xICMS_RET   Picture "@E@Z 999,999,999.99"  // Valor  ICMS Ret.
@ 43, 146  PSAY xVALOR_MERC Picture "@E@Z 999,999,999.99"  // Valor Tot. Prod.

@ 45, 040  PSAY xFRETE      Picture "@E@Z 999,999,999.99"  // Valor do Frete
@ 45, 065  PSAY xSEGURO     Picture "@E@Z 999,999,999.99"  // Valor Seguro
@ 45, 118  PSAY xVALOR_IPI  Picture "@E@Z 999,999,999.99"  // Valor do IPI
@ 45, 146  PSAY xTOT_FAT    Picture "@E@Z 999,999,999.99"  // Valor Total NF
...
@ 47, 006  PSAY xNOME_TRANSP                       // Nome da Transport.

If xTPFRETE == 'C' .or. Empty(xTPFRETE)            // Frete por conta do
	@ 47, 109 PSAY "1"                              // Emitente (1)
Else                                               //     ou
	@ 47, 109 PSAY "2"                              // Destinatario (2)
Endif

@ 47, 097 PSAY " "                                  // Res. p/Placa do Veiculo
@ 47, 122 PSAY " "                                  // Res. p/xEST_TRANSP                          // U.F.

If !EMPTY(xCGC_TRANSP)                              // Se C.G.C. Transportador nao for Vazio
	@ 47, 123 PSAY xCGC_TRANSP Picture"@R 99.999.999/9999-99"
Else
	@ 47, 123 PSAY " "                               // Caso seja vazio
Endif

@ 49, 033 PSAY xEND_TRANSP                          // Endereco Transp.
@ 49, 074 PSAY xMUN_TRANSP                          // Municipio
@ 49, 112 PSAY xEST_TRANSP                          // U.F.
@ 49, 125 PSAY " "                                  // Reservado p/Insc. Estad.

@ 50, 035 PSAY xVOLUME  Picture"@E@Z 999,999.99"             // Quant. Volumes
@ 50, 057 PSAY "PNEU"                                        // Especie
@ 50, 081 PSAY "DIVERSAS"                                    // Res para Marca
@ 50, 085 PSAY " "                                           // Res para Numero
@ 50, 109 PSAY xPESO_BRUTO     Picture"@E@Z 999,999.99"      // Res para Peso Bruto
...
@ 052, 035 PSAY SE4->E4_DESCRI   // Condicao de Pagto

dbSelectArea("SA3")
dbSetOrder(1)
dbSeek(xFilial()+xVEND3)

@ 53, 035 PSAY Alltrim(SA3->A3_NREDUZ)   // Vendedor
...
IF Len(xNUM_NFDV) > 0  .and. !Empty(xNUM_NFDV[1])
	@ 58, 035 PSAY "Nota Fiscal Original No." + "  " + xNUM_NFDV[1] + "  " + xPREF_DV[1]
EndIF

If !Empty(xSuframa)
	@ 59,60 PSAY "SUFRAMA : "+xSuframa
EndIf
    
@ 62, 150 PSAY xNUM_NF                   // Numero da Nota Fiscal
nLin += 5
...
For BB:= 1 to Len(xVALOR_DUP)
	If xDUPLICATAS==.T. .and. BB<=Len(xVALOR_DUP)
		...
		@ nLin, nCol + 15 + nAjuste PSAY xVENC_DUP[BB]
...
For I:=1 to Len(aCOD_PROD)
	IF I == 1
		For _j := 1 To Len(aCOD_PROD)
			IF aCOD_PROD[I,9] !=  "503"
				_nTotal     += aCOD_PROD[I,7]
				xVALOR_MERC := _nTotal
			EndIF
			dbSelectArea("SF4")
			dbSetOrder(1)
			dbSeek(xFilial("SF4")+aCOD_PROD[I,9])
			IF !Empty(SF4->F4_FORMULA)
				IF cCodAnter != aCOD_PROD[I,9]
	...
	IF xCF[I] <> "5916"	  //Caso seja NF de venda
		@ nLin, 040  PSAY aCOD_PROD[I,2]
		@ nLin, 111  PSAY aCOD_PROD[I,3]
		@ nLin, 120  PSAY aCOD_PROD[I,4]
		@ nLin, 126  PSAY aCOD_PROD[I,5]
		@ nLin, 131  PSAY aCOD_PROD[I,6] Picture "@E 99,999,999.99"
		@ nLin, 148  PSAY aCOD_PROD[I,7] Picture "@E 99,999,999.99"
		J:=J+1
	ElseIF I == 1
		For _l :=1 To Len(aCOD_PROD)
			...
			EndIF
		Next _l	
		@ nLin, 040  PSAY Alltrim(Str(xVolume))+" PNEU(s) DEVOLUCAO"
		@ nLin, 111  PSAY aCOD_PROD[I,3]
		@ nLin, 117  PSAY aCOD_PROD[I,4]
		@ nLin, 126  PSAY aCOD_PROD[I,5]
		@ nLin, 131  PSAY aCOD_PROD[I,6] Picture "@E 99,999,999.99"
		@ nLin, 148  PSAY _nTotal        Picture "@E 99,999,999.99"
	EndIF
	nLin :=nLin+1
Next
Return
...
Local I:=1
Local nTamDet  :=16            // Tamanho da Area de Detalhe para os servicos
Local cDesenho := ""
Local cCarcaca := ""
Local cNF_Ser  := ""
...
For I:=1 to nTamDet
	...
	IF I <= Len(aCOD_SERV)
		...
		cNF_Ser  := IIF(!Empty(SC2->C2_SERIEPN),SC2->C2_SERIEPN,"NF-"+SC2->C2_NUMFOGO)
        IF ! alltrim(aCOD_SERV[I,2]) $ "CI/SC"
	        IF aCOD_SERV[i,8] <> "903" .or. Alltrim(aCOD_SERV[i,2]) == "RECUSADO" //Produto recusado
		        cDesc:= cDesenho+" "+Alltrim(aCOD_SERV[I,5])+IIF(!alltrim(aCOD_SERV[I,2]) $ "CI/SC",""," "+Alltrim(SC2->C2_PRODUTO))+" "+Alltrim(cCarcaca)+" "+Alltrim(cNF_Ser)
		    Else 
		...
		@ nLin, 032  PSAY aCOD_SERV[I,3]
		@ nLin, 039  PSAY aCOD_SERV[I,4]
		@ nLin, 061  PSAY cDesc
		IF aCOD_SERV[i,8] == "902"
			nTotDesc := nTotDesc + aCOD_SERV[I,7]
		EndIF
		IF aCOD_SERV[i,8] <> "903" .or. Alltrim(aCOD_SERV[i,2]) == "RECUSADO" //Produto recusado
			@ nLin, 135  PSAY aCOD_SERV[I,6] Picture "@E 99,999,999.99"
			@ nLin, 152  PSAY aCOD_SERV[I,7] Picture "@E 99,999,999.99"
		EndIF	
	EndIF
	nLin :=nLin+1
Next
...
For _k := 1 To Len(xMensITem)
	IF cTesAnt <> xMensITem[_k]
		@ nLin, nCol PSAY Substr(FORMULA(xMensITem[_k]),1,70)
		nLin := nLin +1
		@ nLin, nCol PSAY Substr(FORMULA(xMensITem[_k]),71,70)
		nLin := nLin +1
		@ nLin, nCol PSAY Substr(FORMULA(xMensITem[_k]),141,70)
		nLin := nLin +1
		@ nLin, nCol PSAY Substr(FORMULA(xMensITem[_k]),121,70)
		nLin := nLin +1
