...
AAdd(aRegs,{cPerg,"02","Entrega De?"   ,"Entrega De?"   ,"Entrega De?"   ,"mv_ch2","D", 8,0,0,"G","","mv_par02","","","","","","","","","","","","","","","","","","","","","","","","",""})
AAdd(aRegs,{cPerg,"03","Entrega Ate?"  ,"Entrega Ate?"  ,"Entrega Ate?"  ,"mv_ch3","D", 8,0,0,"G","","mv_par03","","","","","","","","","","","","","","","","","","","","","","","","",""})
AAdd(aRegs,{cPerg,"04","Rodada?"       ,"Rodada?"       ,"Rodada?"       ,"mv_ch4","C", 5,0,0,"G","","mv_par04","","","","","","","","","","","","","","","","","","","","","","","","",""})
...
//-- monta filtro do SC2                   
cFiltro := "DTOS(C2_DATPRF) >= '"+DtoS(mv_par02)+"' "
cFiltro += ".AND. DTOS(C2_DATPRF) <= '"+DtoS(mv_par03)+"' "
...
If !Empty( _aSC5 ) .And. !Empty( _aSC6 )
    ...
    SC5->(dBSetOrder(1))
    IF ! ( SC5->(DBSEEK(XFILIAL("SC5")+_ASC5[1][2],.F.)) )
       NOPCPED := 3 // INCLUI
    ELSE
       FOR I := 1 TO LEN(_ASC6)
           DBSELECTAREA("SC6")
           DBSETORDER(1)
           DBSEEK(XFILIAL("SC6")+_ASC5[1][2],.F.)
           WHILE !EOF() .AND. XFILIAL("SC6") == C6_FILIAL .AND. C6_PEDIDO  == _ASC5[1][2]
              NITEM := C6_ITEM               
              IF VALTYPE(_ASC6[I][16]) = "D" // PRODUTO ORIGINAL PELO IDENTB6
                 IF C6_IDENTB6 == _ASC6[I][16]
                    DBSKIP()
                    LOOP
                 ENDIF
              ELSE
                 IF C6_NUMOP+C6_ITEMOP == _ASC6[I][16]+_ASC6[I][17] // OP PELA OP
                    DBSKIP()
                    LOOP
                 ENDIF
              ENDIF
