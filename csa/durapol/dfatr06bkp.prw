...
ElseIF nOrdem = 7
	IndRegua("TMP",cNomArq,"Descend(Strzero(TMP_COEF,11,2))",,,"Selecionando Registros")
Else
...
While ! Eof()
	...
	Reclock("TMP",.T.)
		...
		Else
			...
			TMP->TMP_LARG         := 0
		EndIF
		TMP->TMP_PRECO   := TRBFAT->D2_TOTAL+TRBFAT->D2_DESCON
		TMP->TMP_DESCO   := TRBFAT->D2_DESCON
		IF TMP->TMP_PRCTB > 0
		...
		Else
			TMP->TMP_DM      := 0
		EndIF
		TMP->TMP_COEF    := ((TRBFAT->D2_TOTAL+TRBFAT->D2_DESCON)*mv_par19) - TMP->TMP_CUSTD
