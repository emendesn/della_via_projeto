...
User Function DESTA1AG () // BY GOLO cAlias, nReg, nOpcx )
	Private aHeader := {}
	Private aAlter  := {}
	Private aCols   := {}
    Private aRotina := {}
	...
	aAdd(aRotina,{"Excluir"   ,"AxExclui",0,5}) 
 	...
 	//            Descrição      Campo         Picture             Tm D Validação                  Tipo
 
	aAdd(aHeader,{"L R"         ,"STATUS"   ,""                   ,01,0,"M->STATUS $ ' GLR'"                                      ,,"C",,})
	aAdd(aHeader,{"Cod.Rej."    ,"MOTREJE"  ,""                   ,02,0,"iif(acols[n,1] = 'R',ExistCpo('SX5','43'+M->MOTREJE),empty(M->MOTREJE))",,"C",,})
	...
	cSql += "   AND C2_X_STATU in (' ','1','2','3','4','5') "
	cSql += " ORDER BY C2_FILIAL,C2_XDTCLAV,C2_XNUMROD,C2_XACLAVE,C2_XBICO"
	...
	aCols := {}
	Do while !eof()     
		aAdd(aCols,{STATUS,MOTREJE,FILIAL,XDTCLAV,XNUMROD,XACLAVE,XBICO,NUM,ITEM,X_STATU,NADA,.F.})
		dbSkip()
	Enddo

	@ 000,000 TO 260,800 DIALOG oDlg TITLE "OK Final ... Libera/Rejeita"
	oGetDados := MSGetDados():New(000,000,078,395,4,"AllwaysTrue()",,,.F.,aAlter,,,Len(aCols),,,,,oDlg)
	@ 100,050 BUTTON "&OK"   SIZE 42,12 ACTION CAutoClave() OBJECT oBtnLibRej
	@ 100,130 BUTTON "&Sair" SIZE 42,12 ACTION Close(oDlg)  OBJECT oBtnSair
...
static function CAutoClave
    for Linha = 1 to len(acols)
    	...
    	dbSetOrder(1)
		if acols[Linha,1] # " "
		   dbSeek(xFilial("SC2")+acols[Linha,8]+acols[Linha,9]+"001")
		   Do case
		   		case acols[Linha,1] = "L"
					 U_DESTA01BL()
				case acols[Linha,1] = "R"
					 U_DESTA01ER()
				case acols[Linha,1] = "G"
		   			 U_DESTA01A("SC2",recno(),4)
		   Endcase
		endif
	next
	Close(oDlg)
...
User Function DESTA01BL( ) // Include by Golo  29/09/06 - FUNCAO LIBERA OP
...
User Function DESTA01b( nOpcx ) // cAlias, nReg, nOpcx,lGLR ) by Golo  29/09/06
...
//by Golo 29/09/06		RetFilter(SC2->(RECNO()))
		Return Nil
	endif
...
//If nOpcx == Nil    			by Golo 29/09
//	oDlgEsp:End()    			by Golo 29/09
//EndIf              			by Golo 29/09

//RetFilter(SC2->(RECNO()))    	by Golo 29/09
// FIM User Function DESTA01b( cAlias, nReg, nOpcx,lGLR ) by Golo
Return Nil
...
PutSX1( cPerg, "01", "Ficha Coleta ?", "", "", "mv_ch1", "C", 6, 0, 1, "G", "", "", "", "", "mv_par01", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "" )
...
cQuery += " AND   C2_NUMD1 = D1_DOC"
cQuery += " AND   C2_ITEMD1 = D1_ITEM
cQuery += " AND   C2_SERIED1 = D1_SERIE"
cQuery += " AND   C2_FORNECE = D1_FORNECE
cQuery += " AND   C2_LOJA = D1_LOJA
	...
	If TR1->TR1_OK == cMarca
		...
		If SF1->( MsSeek( xFilial("SF1") + (cAlias2)->( D1_DOC + D1_SERIE + D1_FORNECE + D1_LOJA + D1_TIPO ) ) ) .And.;
			...
			_aSC5 := 	{ {'C5_FILIAL' , xFilial("SC5") , NIL },;
			              ...
			              {  'C5_LIBEROK', 'S'                , NIL },;
			              {  'C5_CONDPAG', Iif(Empty(SA1->A1_COND), SF1->F1_COND, SA1->A1_COND), NIL } }
