...
User Function DFATR00V3
	...
	Private tamanho        := "M"
	Private nomeprog       := "DFATR00V3"
	Private lAbortPrint    := .F.
	Private nTipo          := 15
	Private aReturn        := { "Zebrado", 1, "Administracao", 1, 1, 1, "", 1}
	Private nLastKey       := 0
	Private cPerg          := "DFTR00V3"
...
Static Function RunReport()
	...
	cSql := ""
	cSql := cSql + "SELECT D2_FILIAL,D2_TOTAL,D2_DESCON,D2_TES,D2_QUANT,D2_XCSBI,d2_doc,d2_item,d2_cod"
	...
	Do While !eof()
		...
		If lRecapa   
			...
			cSql := ""
			cSql := cSql + "SELECT SUM(D3_QUANT) AS QUANT "
			...
			TcSetField("ARQ_CST","QUANT","N",14,2)
			dbSelectArea("ARQ_CST")
			ValVetor(@aKgConsumo,ARQ_SQL->D2_FILIAL,ARQ_CST->QUANT)
			ValVetor(@aCstRecapa,ARQ_SQL->D2_FILIAL,ARQ_SQL->D2_XCSBI)
			dbSelectArea("ARQ_CST")
			dbCloseArea("ARQ_CST")
			dbSelectArea("ARQ_SQL")	
		Endif
