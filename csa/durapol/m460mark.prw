...
±±º          ³                                                            º±±
±±º          ³ Grava variavel "_aTitNcc", publica para ser usada no pontoº±±
...
******>>>   //SC6->C6_TES     := '902'  <<<**********
			...
			*/
			...
			_nValor   := SC6->C6_VALOR + SC6->C6_VALDESC
			_nValDesc := SC6->C6_VALDESC + _nDif  //Retornei o _nDif
			_nDescont := (NoRound(_nValDesc / _nValor , 2) * 100)
			If _nDescont >= 100
				_nDescont := 99.99
			Endif	
