...
_cQry += " D2_TIPO, D2_QUANT, D2_TES, D2_UM, D2_GRUPO, D2_TOTAL, F2_VEND4, F2_VEND5 "
_cQry += " from " + RetSqlName("SF2")+ " SF2, " + RetSqlName("SD2") + " SD2, " + RetSqlName("SA1") + " SA1, " +RetSqlName("SB1")+ " SB1 "
...
IF nOrdem = 1  //Por Produto
	_cQry += " and D2_COD >= '" + mv_par03 + "' and D2_COD <= '" + mv_par04 + "' and "
...
While ! Eof()
	...
	Reclock("TMP",.T.)
		...
		TMP->TMP_DESC    := Alltrim(Substr(TRBFAT->B1_DESC,1,20))
		...
		If lRecapa
			dbSelectArea("SC6")
			dbSetOrder(1)
			dbSeek(xFilial("SC6")+TRBFAT->D2_PEDIDO+TRBFAT->D2_ITEMPV)
			
			dbSelectArea("SD3")
			dbSetOrder(1)
			IF SD3->( dbSeek(xFilial("SD3") + SC6->C6_NUMOP+SC6->C6_ITEMOP+"001" , .f. ) )
				While !Eof() .And. xFilial("SD3") == SD3->D3_FILIAL .And. Alltrim(SD3->D3_OP) == Alltrim(SC6->C6_NUMOP+SC6->C6_ITEMOP+"001")
			...
			Else
				SB1->(dbSetOrder(1))
				SB1->(dbSeek(xFilial("SB1")+TMP->TMP_PROD))
		...
		Else
			TMP->TMP_DM      := 0
		EndIF
		TMP->TMP_COEF    := ((TRBFAT->D2_TOTAL)*mv_par19) - TMP->TMP_CUSTD
...
While ! Eof()
	...
	IF cVar != cCodGen  .or. Bof() // Totalizadores
		...
		nTotVlr := nTotCus := nTotDif := nTtPneu := nTotRec := nTtCons := nVlrCons := nTotTab := 0
