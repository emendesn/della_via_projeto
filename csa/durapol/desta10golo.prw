...
Static Function DST10G (cColeta)
	...
	While !Eof()
		...
		If TR1->TR1_TIPO == "REFORMA"
			...
			If _nValor == 0
				MsgInfo( "O valor serviço " + TR1->TR1_SERV + " Não Cadastrado, o item Não sera gravado no Pedido Venda", "Atenção!" )
				Return Nil
			Endif
			aAux  := {}
			Aadd( aAux, { 'C6_FILIAL' , xFilial("SC6")           , NIL } )
		...
		While !Eof()
			...
			If _nValor == 0
				MsgInfo( "O valor serviço " + TR1->D1_SERVICO + " Não Cadastrado, o item Não sera gravado no Pedido Venda", "Atenção!" )
				Return Nil
			Endif
