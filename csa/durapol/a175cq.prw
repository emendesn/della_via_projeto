...
IF SM0->M0_CODIGO == "03"
	...
	If cItLibRec == 2		
		dbSelectArea("SC6")
		dbSetOrder(1)
		dbSeek(xFilial("SC6")+cNumeroPV)
		While!Eof() .and. xFilial("SC6") == SC6->C6_FILIAL .and. SC6->C6_NUM == cNumeroPV
			SB1->(dbSetOrder(1))
			SB1->(dbSeek(xFilial("SB1")+SC6->C6_PRODUTO))
			IF ( (SC6->C6_NUMOP+SC6->C6_ITEMOP == Substr(cNumeroOP,1,8)) .and. SC6->C6_TES <> '594' .And. SB1->B1_TIPO == 'MO' )
				
				SF4->(dbSeek(xFilial("SF4")+'903',.F.))
				Reclock("SC6",.F.)
					SC6->C6_TES := '903' // cTES // tes para operacoes recusadas
					SC6->C6_CF  := SF4->F4_CF
				MsUnLock()
			EndIF         
			dbSelectArea("SC6")
    ...
    // faturamento
	If !(lEstorno)
    	LiberaItemPV(cNumeroOP,cNumeroPV,cItLibRec) // Libera os itens produzidos e liberados CQ para Faturamento (SC9)
    EndIf
...
Begin Transaction 
	...
	MaAvalSC6("SC6",1,"SC5",.F.,.F.,@lLiberOk,.F.,@lFaturOk) //Reinaldo
End Transaction
...
STATIC FUNCTION LiberaItemPv(cNumeroOP,cNumeroPV,cItLibRec)
...
Begin Transaction
	...
	While!Eof() .and. xFilial("SC6") == SC6->C6_FILIAL .and. SC6->C6_NUM == cNumeroPV
		...
		Reclock("SC6",.F.)
		    SC6->C6_X_FABRI := "S"
		    SC6->C6_TES     := IIF(cItLibRec == 2 .and. SC6->C6_TIPO == "MO","903","901")
		MsUnlock()
	...
	MaAvalSC5("SC5",1,.T.,.F.,@lLiberOk,.F.,@lFaturOk,Nil,Nil,Nil,Nil,.T.) //Reinaldo
