...
IF !Empty(_cServ)
				...
				IF DA0->DA0_ATIVO == "1"
					...
					IF dbSeek(xFilial("DA1")+SA1->A1_TABELA+_cServ)
						...
						If !eof()
							...
							EndIF
							//Else
								//MsgInfo("Nao existe regra de desconto para este cliente","Atenção")
							//EndIF
						Endif
						/*
						IF _nValor == 0 
							_nValor := 0
							_nValor := DA1->DA1_PRCVEN (Solicitado pelo Sr. Samuel)
					   	EndIF
					   	*/
					/*
					Else
    ...
    /*
	IF _nValor == 0
		MsgInfo("Nao existe tabela de preco para este cliente, ou cadastro do produto"+_cServ+" esta sem o preco de venda!","Atenção")
	EndIF
	*/
EndIF
