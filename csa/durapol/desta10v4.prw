...
//  Função			  : Gerar Pedido de Venda 
	...
	AADD(aRegs,{cPerg,"11","Numero de Serie"," "," ","mv_chB","C", 15,0,0,"G","","mv_par11",""          ,"","","","",""       ,"","","","",""             ,"","","","",""            ,"","","","","","","","","   ","","","",""          })
	AADD(aRegs,{cPerg,"12","Numero de Fogo"," "," ","mv_chC","C",06,0,0,"G","","mv_par12",""          ,"","","","",""       ,"","","","",""             ,"","","","",""            ,"","","","","","","","","   ","","","",""          })
	AADD(aRegs,{cPerg,"13","Faturadas?"," "," ","mv_chD","N", 01,0,0,"C","","mv_par13","Nao"          ,"","","","","Sim"       ,"","","","",""             ,"","","","",""            ,"","","","","","","","","   ","","","",""          })
	...
	aadd(aCores,{"FLAG = '1'" ,"BR_VERDE"   })
	aadd(aCores,{"FLAG = '2'" ,"BR_AMARELO" })
	aadd(aCores,{"FLAG = '3'" ,"BR_VERMELHO"})


// Matriz de definição dos menus
//	aAdd(aRotina,{"Pesquisar"    ,"AxPesqui"     ,0,1})    
	aAdd(aRotina,{"Parametros"   ,"U_DST10AP()"  ,0,3})
	aAdd(aRotina,{"Gerar Pedido" ,"U_DST10AX()"  ,0,2})
	dbSelectArea("SC5")
	aAdd(aRotina,{"Pre-Nota"     ,"U_DST10AL()" ,0,1})
	aadd(aRotina,{"Legenda"      ,"U_DST10LG()",0,3})
	Set Key VK_F12 TO U_DST10P()
	MsgRun("Consultando banco de dados...",,{ || AbreTmp() })
	mBrowse(,,,,cAliasCab,aCpos,,,,,aCores,,,,) // Cria o browse
	Set Key VK_F12 TO
// Fecha Temporários
	dbSelectArea("TMP")
	dbCloseArea()
...
Static Function AbreTmp(lSix)
	...
	EndIf
	If !empty(mv_par11)
		cSql += "  AND C2_SERIEPN     = '" + alltrim(mv_par11) + "' "
	endif                                                                            
	if !empty(mv_par12)
		cSql += "  AND C2_NUMFOGO    = '" + alltrim(mv_par12) + "' "
	endif
...
User Function DST10AX ()
	
	U_DESTV02(xFilial("SC2"),TMP->ZZ2_SERIE,TMP->ZZ2_DOC,"ZZ7_COMERC = 'S'")
...
Static Function DST10M (cColeta)
	...
	While !Eof()    
		...
		vMsg := ""
		// Status OP    *** BLOQUEIO DE GERA PEDIDO
		Do Case 
		...
		If !VldCondEsp(xFilial("ZZ2"),TMP->ZZ2_SERIE,TMP->ZZ2_DOC)
			vSTATUS := "Bloqueado"
			vMsg := "Bloqueio comercial"
		Endif
		...
		If RecLock( "TR1", .T. )
			...
			TR1->TR1_TIPO 	:= vTIPO
			TR1->TR1_OK     := iif((vMsg = "OK" .or. substr(vMsg,1,6) = "Aviso!") .and. vSTATUS $ "Liberado*Recusado",cMarca,"")
			MsUnLock() 
		Endif		
...
Static Function DST10G (cColeta)
	...
	dbSetOrder(1)
	dbGoTop()
	dbSeek(xFilial("SA1")+TR1->(TR1_FORNEC+TR1_LOJA))

	nMaxNumCons := SA1->A1_MAXCONS
	nLimConCarc := SA1->A1_LIMCONS

	aEstruCons := {}
	aAdd(aEstruCons,{"ITEM"  ,"C",06,0})
	aAdd(aEstruCons,{"CODIGO","C",15,0})
	aAdd(aEstruCons,{"TES"   ,"C",03,0})
	aAdd(aEstruCons,{"QTDE"  ,"N",09,2})
	aAdd(aEstruCons,{"UNIT"  ,"N",09,2})

	cNomCons2 := CriaTrab(aEstruCons,.T.)
	dbUseArea(.T.,,cNomCons2,"TMPCONS2",.F.,.F.)
	Index on ITEM+CODIGO+TES TAG CONS2A TO &cNomCons2
	...
	aAdd(aEstruCons,{"CODIGO","C",15,0})
	aAdd(aEstruCons,{"TES"   ,"C",03,0})
	aAdd(aEstruCons,{"VALOR" ,"N",09,2})
	aAdd(aEstruCons,{"MAX"   ,"N",03,0})
	aAdd(aEstruCons,{"RECUSA","C",01,0})
	
	cNomCons1 := CriaTrab(aEstruCons,.T.)
	dbUseArea(.T.,,cNomCons1,"TMPCONS",.F.,.F.)
	Index on Descend(StrZero(VALOR*100,8))+ITEM TAG CONS1A TO &cNomCons1
	Index on ITEM+CODIGO+TES TAG CONS1B TO &cNomCons1

	dbSelectArea("TMPCONS")
	dbSetOrder(1)
	dbGoTop()
	...
	dbGoTop()
	do while .not. eof()
		If !(TR1->TR1_OK = cMarca)  // Ignora ITEM DESMARCADO
			dbSkip()
			Loop
		Endif                        
		...
		// Busca consertos
		cSql := "SELECT SUBSTR(ZF_NUMOP,1,8) AS ITEM"
		cSql += " ,      ZF_PRODSRV AS CODIGO"
		cSql += " ,      ZF_QUANT   AS QUANT"
		cSql += " ,      (CASE WHEN ACP_PRCVEN IS NULL THEN DA1_PRCVEN ELSE ACP_PRCVEN END) AS PRC"
		...
		cSql += " AND  ACP_CODREG = '"+TR1->TR1_FORNEC+"'"
		cSql += " AND  ACP_CODPRO = ZF_PRODSRV"

		cSql += " LEFT JOIN "+RetSqlName("DA1")+" DA1"
		cSql += " ON   DA1.D_E_L_E_T_ = ''"
		cSql += " AND  DA1_FILIAL = '"+xFilial("DA1")+"'"
		cSql += " AND  DA1_CODTAB = '001'"
		cSql += " AND  DA1_CODPRO = ZF_PRODSRV"

		cSql += " WHERE SZF.D_E_L_E_T_ = ''"
		cSql += " AND   ZF_FILIAL = '"+xFilial("SZF")+"'"
		cSql += " AND   ZF_NUMOP = '"+TR1->(TR1_NUM+TR1_ITEM)+"001'"

		cSql += " UNION"

		cSql += " SELECT SUBSTR(D3_OP,1,8) AS ITEM"
		cSql += " ,      D3_X_PROD AS CODIGO"
		cSql += " ,      D3_QUANT   AS QUANT"
		cSql += " ,      (CASE WHEN ACP_PRCVEN IS NULL THEN DA1_PRCVEN ELSE ACP_PRCVEN END) AS PRC"
		...
		cSql += " AND  ACP_FILIAL = '"+xFilial("ACP")+"'"
		cSql += " AND  ACP_CODREG = '"+TR1->TR1_FORNEC+"'"
		cSql += " AND  ACP_CODPRO = D3_COD"

		cSql += " LEFT JOIN "+RetSqlName("DA1")+" DA1"
		cSql += " ON   DA1.D_E_L_E_T_ = ''"
		cSql += " AND  DA1_FILIAL = '"+xFilial("SA1")+"'"
		cSql += " AND  DA1_CODTAB = '001'"
		cSql += " AND  DA1_CODPRO = D3_X_PROD"

		cSql += " WHERE SD3.D_E_L_E_T_ = ''"
		cSql += " AND   D3_FILIAL = '"+xFilial("SD3")+"'"
		cSql += " AND   D3_OP = '"+TR1->(TR1_NUM+TR1_ITEM)+"001'"
		cSql += " AND   D3_GRUPO = 'MANC'"

		dbUseArea(.T., "TOPCONN", TCGenQry(,, cQdb2), "SQL_CONS", .F., .T. ) 
		TCSetField("SQL_CONS","QUANT","N",14,2)
		TCSetField("SQL_CONS","PRC"  ,"N",14,2)
				
		// Desmenbra e grava no temporário
		dbSelectArea("SQL_CONS")
		dbGoTop()
		Do While !eof()
			dbSelectArea("TMPCONS")
			For k=1 to INT(SQL_CONS->QUANT)
				If RecLock("TMPCONS",.T.)
					TMPCONS->ITEM   := SQL_CONS->ITEM
					TMPCONS->CODIGO := SQL_CONS->CODIGO
					TMPCONS->VALOR  := SQL_CONS->VALOR
					TMPCONS->MAX    := nLimConCarc
					TMPCONS->RECUSA := iif(Upper(AllTrim(TR1->TR1_STATUS)) = "RECUSADO","S","N")
					MsUnLock()
				Endif
			Next k
	...
	dbSelectArea("TMPCONS")
	dbGoTop()
	REPLACE ALL TES WITH "903" FOR RECUSA = "S"

	// Seleciona os mais caros a serem cobrados
	For k=1 to nMaxNumCons
		dbSelectArea("TMPCONS")
		...
		dbGoTop()
		Do While !eof()
			If Empty(TES) .and. MAX > 0	
				If RecLock("TMPCONS",.F.)
					TMPCONS->TES := "901"
					MsUnLock()
				Endif
				cChave := ITEM
				...
				dbGoTop()
				REPLACE ALL MAX WITH MAX-1 FOR ITEM = cChave
	...
	dbSelectArea("TMPCONS")
	dbGoTop()
	REPLACE ALL TES WITH "902" FOR Empty(TES)


	// Aglutina itens
	dbSetOrder(2) // ITEM+CODIGO+TES
	dbGoTop()

	Do While !eof()
		aChave := {ITEM,CODIGO,TES}
		nUnit  := VALOR
		nQuant := 0

		Do While !eof() .AND. ITEM = aChave[1] .AND. CODIGO = aChave[2] .AND. TES = aChave[3]
			nQuant++
			dbSkip()
		Enddo

		dbSelectArea("TMPCONS2")
		If RecLock("TMPCONS2",.T.)
			TMPCONS2->ITEM   := aChave[1]
			TMPCONS2->CODIGO := aChave[2]
			TMPCONS2->TES    := aChave[3]
			TMPCONS2->QTDE   := nQuant
			TMPCONS2->UNIT   := nUnit
			MsUnLock()
		Endif
	...
	Enddo
	dbCloseArea()
	fErase(cNomCons1+GetDbExtension())
	fErase(cNomCons1+OrdBagExt())
    ...
    While !Eof()		                            
		...
		If Alltrim(TR1->TR1_Status) == "Recusado"
			cDescPro := AllTrim(TR1->TR1_TIPO) + " " + Alltrim(TR1->TR1_COD) + " " + Alltrim(TR1->TR1_CARCAC)
			cDescPro += Iif( !Empty( TR1->TR1_NUMFOG ), " NF-" + Alltrim( TR1->TR1_NUMFOG ) , "")  
			cDescPro += Iif( !Empty( TR1->TR1_SERIEP ), " NS-" + Alltrim( TR1->TR1_SERIEP ) , "")
			_nValor  := U_BuscaAcp ( TR1->TR1_SERV, TR1->TR1_FORNEC, TR1->TR1_LOJA ) 
			aAux  := {}
		...
		// Fatura Só Conserto
		If Alltrim(TR1->TR1_Status) == "Liberado" .and. Alltrim(TR1->TR1_TIPO) == "SOCONSERTO"
			_nValor  := U_BuscaAcp ( TR1->TR1_SERV, TR1->TR1_FORNEC, TR1->TR1_LOJA )		
			SB1->( MsSeek( xFilial("SB1") + TR1->TR1_SERV ) )
			SF4->( MsSeek( xFilial("SF4") + "901" ) )
			...
			Aadd( aAux, { 'C6_NUMOP'  , TR1->TR1_NUM             , NIL } )
			Aadd( aAux, { 'C6_ITEMOP' , TR1->TR1_ITEM            , NIL } )
			Aadd( aAux, { 'C6_TES'    , "901"                    , NIL } )
			Aadd( aAux, { 'C6_CF'     , SF4->F4_CF               , NIL } )
			Aadd( _aSC6, aAux )
		Endif


		// Fatura Conserto - Novo
		dbSelectArea("TMPCONS2")
		dbSetOrder(2) // ITEM+CODIGO+TES
		dbGoTop()
		
		Do While !eof() .AND. TMPCONS2->ITEM = TR1->(TR1_NUM+TR1_ITEM)
			SB1->( MsSeek( xFilial("SB1") + TMPCONS2->CODIGO ) )
			SF4->( MsSeek( xFilial("SF4") + TMPCONS2->TES ) )

			aAux  := {}
			Aadd( aAux, { 'C6_FILIAL' , xFilial("SC6")           , NIL } )
			Aadd( aAux, { 'C6_NUM'    , cCOLETA                  , NIL } )
			Aadd( aAux, { 'C6_ITEM'   , StrZero(Len(_aSC6)+1,2,0), NIL } )
			Aadd( aAux, { 'C6_PRODUTO', TMPCONS2->CODIGO         , NIL } )
			Aadd( aAux, { 'C6_DESCRI' , SB1->B1_DESC             , NIL } )
			Aadd( aAux, { 'C6_QTDVEN' , TMPCONS2->QTDE           , NIL } )
			Aadd( aAux, { 'C6_QTDLIB' , TMPCONS2->QTDE           , NIL } )
			Aadd( aAux, { 'C6_PRCVEN' , TMPCONS2->UNIT           , NIL } )
			Aadd( aAux, { 'C6_PRUNIT' , TMPCONS2->UNIT           , NIL } )
			Aadd( aAux, { 'C6_VALOR'  , TMPCONS2->(QTDE*UNIT)    , NIL } )
			Aadd( aAux, { 'C6_LOCAL'  , "01"                     , NIL } )
			Aadd( aAux, { 'C6_NFORI'  , ""                       , NIL } )
			Aadd( aAux, { 'C6_SERIORI', ""                       , NIL } )
			Aadd( aAux, { 'C6_ITEMORI', ""                       , NIL } )
			...
			Aadd( aAux, { 'C6_NUMOP'  , TR1->TR1_NUM             , NIL } )
			Aadd( aAux, { 'C6_ITEMOP' , TR1->TR1_ITEM            , NIL } )
			Aadd( aAux, { 'C6_TES'    , TMPCONS2->TES            , NIL } )
			Aadd( aAux, { 'C6_CF'     , SF4->F4_CF               , NIL } )
			Aadd( _aSC6, aAux )

			dbSkip()
		Enddo
		dbCloseArea()  //TMPCONS2
		fErase(cNomCons2+GetDbExtension())
		fErase(cNomCons2+OrdBagExt())


		/*
		// Fatura Conserto - Velho
...
User Function DST10AL()
	...
	wnrel := SetPrint(cString,"DST10AL",cPerg,@titulo,cDesc1,cDesc2,cDesc3,.T.,aOrd,.F.,Tamanho,,.F.)
...
Static Function VldCondEsp(cParFil,cParSer,cParDoc)
	Local aArea  := GetArea()
	Local lRet   := .F.
	Local cSql   := ""
	Local cAlias := GetNextAlias()

	cSql := "SELECT ZZ6_CODIGO FROM "+RetSqlName("ZZ6")
	cSql += " WHERE D_E_L_E_T_ = ''"
	cSql += " AND   ZZ6_FILIAL = '"+cParFil+"'"
	cSql += " AND   ZZ6_SERIE = '"+cParSer+"'"
...
Static Function TotParc(cParFil,cParOP)
	Local aArea  := GetArea()
	Local k      := 0
	Local cRet   := ""
	Local cSql   := ""
	...
	For k=1 to 2
		cSql := "SELECT COUNT(C2_FILIAL) AS QTD"
		cSql += " FROM "+RetSqlName("SC2")
		cSql += " WHERE D_E_L_E_T_ = ''"
		cSql += " AND   C2_FILIAL = '"+cParFil+"'"
		cSql += " AND   C2_NUM = '"+cParOP+"'"
		If k=1
			cSql += " AND   C2_X_STATU IN('6','9')"
		Else
			cSql += " AND   C2_X_STATU NOT IN('6','9')"
		Endif
		dbUseArea(.T., "TOPCONN", TCGenQry(,,cSql),cAlias, .T., .T.)
		TcSetField(cAlias,"QTD","N",6,0)
		dbSelectArea(cAlias)
		If k=1
			nLib := QTD
		Else
	...
	If nLib > 0 .AND. nProd = 0
		cRet := "1" // Total
	Elseif nLib > 0 .AND. nProd > 0
		cRet := "2" // Parcial
	Else
		cRet := "3" // Produção
	Endif
...
Static Function Flags()
	...
	Do While !eof()
		If RecLock("TMP")
			TMP->FLAG := TotParc(xFilial("SC2"),TMP->C2_NUM)
			TMP->BLQ  := iif(VldCondEsp(xFilial("SC2"),TMP->ZZ2_SERIE,TMP->ZZ2_DOC),"Não","Sim")
			MsUnLock()
		Endif
		dbSkip()
	Enddo
...
User Function DST10LG()
	BrwLegenda(cCadastro,"Legenda", {{"BR_VERDE"   ,"Total"   },;
									 {"BR_AMARELO" ,"Parcial" },;
									 {"BR_VERMELHO","Produção"}})
Return nil