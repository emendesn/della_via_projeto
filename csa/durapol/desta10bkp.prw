...
Static Function DST10G (cColeta)
	...
	Select A1_VEND3 from SF1030 SF1, 
	where SF1.D_E_L_E_T_ 	= ''
	  and SF1.DOC			=
	cQuery := "SELECT F1_DOC, "
	cQuery += "  FROM " + RetSqlName("SF1") + " SF1, "
	cQuery += " WHERE D_E_L_E_T_ = ' ' " 
	cQuery += "   AND F1_FILIAL  = '" + TR1->TR1_FILIAL + "' "
	cQuery += "   AND F1_FORNECE = '" + TR1->TR1_FORNEC	+ "' "
	cQuery += "   AND F1_LOJA    = '" + TR1->TR1_LOJA   + "' "
	cQuery += "   AND F1_DOC     = '" + TR1->TR1_NUMD1  + "' "
	cQuery += "   AND F1_SERIE   = '" + TR1->TR1_SERIE  + "' "
