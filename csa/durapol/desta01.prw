...
User Function DESTA1AG () // BY GOLO cAlias, nReg, nOpcx )
	...
	aAdd(aHeader,{"L R"         ,"STATUS"   ,"@!"                 ,01,0,"M->STATUS $ ' LRGV'"                                      ,,"C",,})
	aAdd(aHeader,{"Cod.Rej."    ,"MOTREJE"  ,""                   ,02,0,"iif(acols[n,1] = 'R',ExistCpo('SX5','43'+M->MOTREJE,1),empty(M->MOTREJE))",,"C",,})
	...
	aCols := {}
	Do while !eof()     
		aAdd(aCols,{STATUS,MOTREJE,NUM,ITEM,XDTCLAV,XNUMROD,XACLAVE,XBICO,X_STATU,FILIAL,NADA,.F.})
		dbSkip()
	Enddo

	@ 000,000 TO 260,800 DIALOG oDlg TITLE "OK Final ... Libera/Rejeita"
	oGetDados := MSGetDados():New(000,000,078,395,4,"U_DST01LOK()",,,.F.,aAlter,,,Len(aCols),,,,,oDlg)
...
User Function DST01LOK()
	Local lRet := .T.
	
	If aCols[n,1]="R" .AND. Empty(aCols[n,2])
		lRet := .F.
	Endif
Return lRet
...
While (cAlias1)->( !Eof() )
	...
	*/
	...
	TR1->TR1_CLIENT  := (cAlias1)->C2_XNREDUZ   //SA1->A1_NREDUZ
...
If TR1->( !Eof() )
	...
	If nOpca == 1
		...
		MsgRun("Gerando pedido...",,{ || U_DESTA01g(cColeta) })
	EndIf
EndIf
...
User Function DESTA01h()
	MsgStop( "Filtro não dever ser utilizado")
Return nil

User Function Desta01AGA() // Versão anterior do DESTA01h()
Local cPerg   := 'ESTA1A'
Local aRegs   := {}
Local nIndex
