...
Static Function RunReport()
	...
	CABEC1="FD Coleta Ser Item Client Lj Nome do Cliente      Entrega  OrdemP It Status     Serviço    Emiss.OP AutoClav Producao  Desenho         Carcaça         Fogo   Série"
	CABEC2="                                                                                           Cond.Pagt. Vend3  Vend4  Vend5  CC          Medida          Observação"
    vReg := ""
	While !Eof()
		   	...
		   	If SQL3->GRUPO = 'BAND'
		   		...
		   		If vTIPO = "RECAPAGEM" .and. !("EXAM" $ Arq_Db2->D1_X_Desen)                                        
					...
					If StrTran(StrTran(vDesLan,"-","")," ","") <> StrTran(StrTran(vDesCol,"-","")," ","")  
						aAdd(aTab,"Banda Lançada na OP-Desenho("+vDesLan+")Diferente do Desenho Informado na Coleta("+vDesCol+")"+" "+SQL3->NUMOP+" "+SQL3->X_PROD) 
						TudoOk  := .F.                         
					EndIf
				EndIf
			EndIf 
