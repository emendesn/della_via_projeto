...
While !Eof().And.(x3_arquivo=="SD1")
	If X3USO(x3_usado) .AND. Alltrim(X3_CAMPO) == "D1_ITEM" .OR. Alltrim(X3_CAMPO) == "D1_SERVICO" .OR. Alltrim(X3_CAMPO) == "D1_SERIEPN" .OR. Alltrim(X3_CAMPO) == "D1_NUMFOGO" .OR. ;
	   Alltrim(X3_CAMPO) == "D1_MARCAPN" .OR. Alltrim(X3_CAMPO) == "D1_CARCACA" .OR. Alltrim(X3_CAMPO) == "D1_X_DESEN" ;
	   .OR.  Alltrim(X3_CAMPO) == "D1_TOTAL" .OR. Alltrim(X3_CAMPO) == "D1_VUNIT"  .OR. Alltrim(X3_CAMPO) == "D1_COD" .OR. Alltrim(X3_CAMPO) == "D1_IDENTB6"
	   nUsado:=nUsado+1
	   Aadd(aHeader,{ TRIM(x3_titulo), x3_campo, x3_picture,x3_tamanho, x3_decimal,"AllwaysTrue()",;
