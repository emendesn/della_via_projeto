...
±±º          ³                                                            º±±
±±º          ³ Usa variavel "_aTiTNcc      ", publica definida no ponto deº±±
...
//-- verifica qual e' o cliente dos itens selecionados para a nota
If Len(_aItPed)>0
	dbSelectArea("SC6")
	dbGoTo(_aItPed[1,10])  // posiciona do registro do SC6 ( no primeiro )
	_cCliente := SC6->C6_CLI
	_cLoja    := SC6->C6_LOJA
Endif

//-- pega conteudo da variavel publica definida no ponto de entrada M460MARK ( titulos NCC com saldo para o cliente )
//-- filtra somente os titulos do cliente que veio nos itens selecionados
...
aTitulos := {}
For _nX:=1 To Len(_aTiTNcc)
	If _aTiTNcc[_nX,13] == _cCliente .And. _aTiTNcc[_nX,14] == _cLoja
		AAdd(aTitulos,_aTiTNcc[_nX])
	Endif
Next		

//--- Daqui para frente processa somente os titulos do cliente e os itens do cliente
//--- itens da nota sao de um unico cliente
...
If _lOk
	...
	For _nX := 1 To Len(_aItPed)
		...
		//-- acerto do para quando o preco ficou zerado
		If SC6->C6_PRCVEN == 0.01
			_nTotDesc += SC6->C6_VALOR
		Endif
	Next	
