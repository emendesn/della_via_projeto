...
//Ё mv_par03 - Gerar Lancamentos Por        ?  NF  Periodo Dia   Ё
			...
			If lValido
				...
				If nTpCtb == 1 .And. lHeader
					...
					If nTotalCtb > 0 				
						...
						SD2->(dbGoTo(nUltSD2))		// POSICIONA NO зLTIMO SD2 DA NF.
						nTotalCtb := 0
...
╠╠ЁFun┤└o    ЁACertaSX1     Ё Autor Ё Patricia A. Salomao  Ё Data Ё26.07.2005Ё╠╠
╠╠цддддддддддеддддддддддддддадддддддаддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o ЁAcerta o SX1 : Inclui a opcao 'Dia' no Combobox do mv_par03    Ё╠╠
╠╠цддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe   ЁACertaSX1()                                                    Ё╠╠
╠╠цддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁParametrosЁExpC1 - Nome da Pergunte                                       Ё╠╠
╠╠цддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁRetorno   Ё.T.                                                            Ё╠╠
