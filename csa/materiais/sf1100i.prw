...
User Function SF1100I()
	...
	If cEmpAnt == "01"  // DELLA VIA
		...
		While !EOF() .And. D1_FILIAL == xFilial("SD1") .And. D1_COD == cPrdSEPU .And. D1_DOC == SF1->F1_DOC .And. D1_SERIE == SF1->F1_SERIE
		...
		If SF1->F1_TIPO = "B"
			...
			nValNCC := 0
			Do While !eof() .AND. SD1->D1_DOC = SF1->F1_DOC .AND. SD1->D1_SERIE = SF1->F1_SERIE
				If AllTrim(D1_COD) $ "102020*102021*102070"
					nValNCC += D1_TOTAL
				Endif
				dbSkip()
			enddo
			...
			If nValNCC > 0
				IF RecLock("SE1",.T.)
					...
					Replace E1_SERIE    with SF1->F1_SERIE
					Replace E1_NUM	    with SF1->F1_DOC
					Replace E1_PARCELA  with "A"
					Replace E1_NATUREZ  with "2002"
					Replace E1_TIPO	    with "NCC"
					Replace E1_EMISSAO  with dDataBase
					Replace E1_VALOR    with nValNCC
					...
					Replace E1_VLCRUZ   with Round(NoRound(xMoeda(SE1->E1_VALOR,1,1,SE1->E1_EMISSAO,MsDecimais(1)+1),MsDecimais(1)+1),MsDecimais(1))
