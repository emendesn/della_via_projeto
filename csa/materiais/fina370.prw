...
╠╠ЁFun┤└o	 Ё FINA370	Ё Autor Ё Wagner Xavier 		  Ё Data Ё 24/08/94 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o Ё Programa de Lan┤amentos Cont═beis Off-Line					  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 Ё FINA370()																  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁUso		 Ё SIGAFIN																	  Ё╠╠
...
//Ё mv_par01 // Mostra Lan┤amentos Cont═beis 						  Ё
//Ё mv_par02 // Aglutina Lan┤amentos Cont═beis						  Ё
//Ё mv_par03 // Emissao / Data Base 									  Ё
//Ё mv_par04 // Data Inicio												  Ё
//Ё mv_par05 // Data Fim													  Ё
//Ё mv_par06 // Carteira : Receber / Pagar /Cheque / Ambas 		  Ё
//Ё mv_par07 // Baixas por Data de Emiss└o ou Digita┤└o			  Ё
//Ё mv_par08 // Considera filiais abaixo                         Ё
//Ё mv_par09 // Da Filial                                        Ё
//Ё mv_par10 // Ate a Filial                                     Ё
//Ё mv_par11 // Atualiza Sinteticas                              Ё
//Ё mv_par12 // Separa por ? (Periodo,Documento,Processo)        Ё
//Ё mv_par13 // Ctb Bordero - Total/Por Bordero                  Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбдддддддддд©╠╠
╠╠ЁFun┤└o	 ЁFA370Proc Ё Autor Ё Wagner Xavier 		  Ё Data Ё 24/08/94 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o Ё Programa de Lan┤amentos Cont═beis Off-Line					  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 Ё FINA370()																  Ё╠╠
...
While !Eof() .and. M0_CODIGO == cEmpAnt .and. M0_CODFIL <= cFilAte
	...
	If lLerSe5
		...
		//Ё Ponto de Entrada para filtrar registros do SE5. Ё
	...
	//Ё Contabiliza pelo E2_EMIS1 - CONTAS PAGAR   			  Ё
	...
	If (mv_par06 = 2 .Or. mv_par06 = 4) .and. lLerSe2
      ...
      //зддддддддддддддддддддддддддддддддддддддддддддддддд©
      //Ё Ponto de Entrada para filtrar registros do SE2. Ё
      //юддддддддддддддддддддддддддддддддддддддддддддддддды
      If l370E2FIL
	...
	If (mv_par06 = 3 .Or. mv_par06 = 4 ) .and. lLerSef
      ...
      //зддддддддддддддддддддддддддддддддддддддддддддддддд©
      //Ё Ponto de Entrada para filtrar registros do SEF. Ё
      //юддддддддддддддддддддддддддддддддддддддддддддддддды
      If l370EFFIL
	...
	//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё La┤o da contabiliza┤└o dia a dia, pela emiss└o (mv_par03 = 1)Ё
	//Ё ou pela database.														  Ё
	//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	If mv_par03 == 1
	...
	BEGIN SEQUENCE
		...
		For nLaco := 1 to nPeriodo
			...
			//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			//Ё Se a contabiliza┤└o for pela data de emiss└o, altera o valor Ё
			//Ё da data-base e dos par┐metros, para efetuar a contabiliza┤└o Ё
			//Ё e a sele┤└o dos registros respectivamente.						  Ё
			//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
			If mv_par03 == 1
			...
			//Ё Verifica se ┌ um EXECBLOCK e caso sendo, executa-o							Ё
			...
			If (mv_par06 == 1 .or. mv_par06 == 4) .and. lLerSe1
               ...
               //зддддддддддддддддддддддддддддддддддддддддддддддддд©
               //Ё Ponto de Entrada para filtrar registros do SE1. Ё
               //юддддддддддддддддддддддддддддддддддддддддддддддддды
               If l370E1FIL
               ...
               EndIf
					...
					//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Verifica se ser═ gerado Lan┤amento Cont═bil			  Ё
					//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
					...
					//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Posiciona na natureza.										  Ё
					//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
					dbSelectArea( "SED" )
					dbSeek( cFilial+SE1->E1_NATUREZ )

					//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Posiciona na SE5,se RA										  Ё
					//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
				...
				If lLerSE5
					//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Movimenta┤└o Banc═ria a Receber - SE5990 						  Ё
					//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
					dbSelectArea( "SE5" )
					dbSetOrder( 1 )
					dbSeek( cFilial+Dtos(mv_par04),.T. )
					...
					While !Eof() .And. cFilial == SE5->E5_FILIAL .And. ( SE5->E5_DATA >= mv_par04 ;
						...
						If SE5->E5_TIPODOC $ "AP/RF/PE/EP"
							...
							EndIf
							//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
							//Ё Posiciona Registros para a baixa             		  Ё
							//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
							dbSelectArea("SEH")
							dbSetOrder(1)
						...
						//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
						//Ё Caracteriza-se lancamento os registros com :		  Ё
						//Ё E5_TIPODOC = brancos                         		  Ё
						//Ё E5_TIPODOC = "DB" // Receita Bancaria - FINA200     Ё
						//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
						If ( !lX )
			...
			If (mv_par06 == 2 .or. mv_par06 == 4) .and. lLerSE2
				...
				While !Eof() .And. xFilial("SE2") == E2_FILIAL .And. E2_EMIS1 >= mv_par04 ;
					...
					//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Posiciona na SE5 e no Banco,se PA e SEF para Cheque Ё
					//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
					If SE2->E2_TIPO $ MVPAGANT
						dbSelectArea("SE5")
						dbSetOrder(2)
				...
				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё Movimenta┤└o Banc═ria a Pagar - SE5990							  Ё
				//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				dbSelectArea( "SE5" )
				dbSetOrder( 1 )
				dbSeek( cFilial+Dtos(mv_par04),.T. )
					...
					If !lX
						If !Empty(SE5->E5_TIPODOC) .Or.;
							...
							If !(SE5->E5_TIPODOC $ "DB/OD")
								...
								//Ё Caracteriza-se lancamento os registros com :		  Ё
								//Ё E5_TIPODOC = brancos                         		  Ё
								//Ё E5_TIPODOC = "DB" // Desp Bancaria gerada no FINA200Ё
								//Ё E5_TIPODOC = "OD" // Outras Despesas gerada FINA200 Ё
								//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
								SE5->( dbSkip())
								Loop
							EndIF
			...
			If (mv_par06 == 1 .or. mv_par06 == 4) .and. lLerSE5
				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё Baixas a Receber 														  Ё
				//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				dbSelectArea( "SE5" )

				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё Verifica qual a data a ser utilizada para baixa				  Ё
				//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				...
				While ! SE5->(Eof()) .and. SE5->E5_RECPAG == "R" .and. &cCampo <= mv_par05
					...
					//Ё Nao serao contabilizadas Mov. Fin. Manuais e Transf.Bancaria neste ponto  Ё
					...
					If (lAdiant .or. lEstorno) .and. !lEstRaNcc
						...
						If !(dbSeek(cFilial+SE5->E5_PREFIXO+SE5->E5_NUMERO+SE5->E5_PARCELA+SE5->E5_TIPO+SE5->E5_CLIFOR+SE5->E5_LOJA))
							...
							//Ё Localizada inconsist┬ncia no arquivo SE5. A fun┤└o fa370conc	 Ё
							//Ё pergunta se o usu═rio quer continuar ou abandonar.				 Ё
					...
					While !Eof() .And. &cChaveSe5
						...
						If  SE5->E5_TIPODOC $ "BAЭVLЭV2ЭES/LJ"
							nVl	  := E5_VALOR
							...
							cSeqSE5	:= SE5->E5_SEQ
						ElseIf SE5->E5_TIPODOC $ "DCЭD2"
							nDc	  := E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						ElseIf SE5->E5_TIPODOC $ "JRЭJ2ЭTL"
							nJr	  := E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						ElseIf SE5->E5_TIPODOC $ "MTЭM2"
							nMt := E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						ElseIf SE5->E5_TIPODOC $ "CMЭC2"
							nCm := E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						Endif
					...
					//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Posiciona no cliente/fornecedor 						  Ё
					//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
					If lTitulo
						If (lAdiant .or. lEstorno) .and. !lEstRaNcc
							dbSelectArea("SA2")
					...
					//Ё Posiciona o arquivo SE5 para que os lan┤amentos  Ё
					//Ё cont═beis possam localizar o motivo da baixa.	  Ё
					...
					//Ё Devolve a posi┤└o original do arquivo  Ё
			...
			If (mv_par06 == 2 .or. mv_par06 == 4) .and. lLerSE5
				...
				While !SE5->(Eof()) .And. SE5->E5_RECPAG == "P" .and. &cCampo <= mv_par05
					...
					//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
					//Ё Apenas contabilza se :                                       Ё
					//Ё For realmente uma baixa de contas a PAGAR                    Ё
					//Ё mv_ctbaixa diferente de "C" - Cheque
					//Ё_______________________________________________________________
					...
					While  !Eof() .And. &cChaveSE5
						...
						If	SE5->E5_TIPODOC $ "BAЭVLЭV2ЭES/LJ/E2"
							cBanco  := SE5->E5_BANCO+SE5->E5_AGENCIA+SE5->E5_CONTA
						...
						ElseIf SE5->E5_TIPODOC $ "DCЭD2"
							nDc	  := SE5->E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						ElseIf SE5->E5_TIPODOC $ "JRЭJ2/TL"
							nJr	  := SE5->E5_VALOR
							If nRegSE5 == 0; nRegSE5 := SE5->(Recno()); Endif
						ElseIf SE5->E5_TIPODOC $ "MTЭM2"
							nMt := SE5->E5_VALOR
...
// Disponibilizo a Variavel VALOR com o total dos borderos aglutinados
			...
			If (mv_par06 = 3  .Or. mv_par06 = 4) .and. lLerSEF
				...
				While !Eof() .And. xFilial("SEF") == SEF->EF_FILIAL
					...
					//Ё Guarda o pr╒ximo registro para IndR┌gua					  Ё
					...
					If SEF->EF_DATA >= mv_par04 .And. SEF->EF_DATA <= mv_par05 .And. !Empty(SEF->EF_NUM) .and. ;
						...
						If SEF->EF_IMPRESS $ "SN "	// Cheque impresso ou n└o
							...
							//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
							//Ё Antes de desposicionar deve gravar flag de contabi- Ё
							//Ё lizado.                                             Ё
							//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
							Reclock("SEF")
							SEF->EF_LA := "S"
							MsUnlock()
							//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
							//Ё Desposiciona propositalmente o SEF para que APENAS aЁ
							//Ё variavel VALOR esteja com conteudo. O reposicionamenЁ
							//Ё to ┌ feito na volta do Looping.                     Ё
							//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
							If Alltrim(SEF->EF_ORIGEM) == "FINA190"  // Jun┤фo de cheques
								dbSelectArea("SEF")
								dbGoBottom()
								dbSkip()
								dbSelectArea("SE1")
						...
						If !SEF->(EOF())
							...
							If !lChqSTit
								If SEF->EF_TIPO $ MVRECANT + "/" + MV_CRNEG
									...
									//Ё Neste caso o titulo veio de um ContasЁ
									//Ё a Receber (SE1)                      Ё
				...
				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё Movimenta┤└o Banc═ria - Transferencia & Estorno da Transf.   Ё
				//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				aTrf := {"TR","TE"}
			...
			If mv_par06 != 3 .and. lLerSEU
				...
				While !Eof() .And. cFilial == SET->ET_FILIAL
	               ...
	               //зддддддддддддддддддддддддддддддддддддддддддддддддд©
	               //Ё Ponto de Entrada para filtrar registros do SEU. Ё
	               //юддддддддддддддддддддддддддддддддддддддддддддддддды
	               If l370EUFIL
	               ...
	               EndIf
						...
						// Tipo 00 com Nro de adiantamento = PrestaГЦo de contas (R)
	...
	//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё Data inicial precisa ser "resetada"                 Ё
	//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Recupera o valor real da data base por seguranca	  Ё
//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
dDataBase := dDataAnt

//зддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Recupera a filial original                      	  Ё
//юддддддддддддддддддддддддддддддддддддддддддддддддддддды
SM0->(dbGoto(nRegEmp))
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбдддддддддд©╠╠
╠╠ЁFun┤└o	 Ё FA370CONCЁ Autor Ё Vinicius Barreira	  Ё Data Ё 24/08/95 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o Ё Tela de Aviso de Falha na consist┬ncia do SE5				  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 Ё FA370CONC() 															  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁUso		 Ё SIGAFIN																	  Ё╠╠
╠╠юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды╠╠
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбдддддддддд©╠╠
╠╠ЁFun┤└o	 Ё Fa370Rat Ё Autor Ё Pilar S. Albaladejo   Ё Data Ё 25/08/95 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o Ё Grava linha de rateio no arquivo de contra-prova			  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 Ё Fa370Rat()																  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁUso		 Ё SIGAFIN																	  Ё╠╠
╠╠юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды╠╠
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбдддддддддд©╠╠
╠╠ЁFun┤└o	 Ёa370CabecaЁ Autor Ё Wagner Xavier 		  Ё Data Ё 24/08/94 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o ЁMonta o arquivo de Contra Prova (Lancamentos off line) 	  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 Ёa370Cabeca																  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁUso		 ЁSIGAFIN																	  Ё╠╠
╠╠юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды╠╠
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбдддддддддд©╠╠
╠╠ЁFun┤└o	 ЁCa370Incl Ё Autor Ё Claudio D. de Souza	  Ё Data Ё 12/08/02 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддадддддддддд╢╠╠
╠╠ЁDescri┤└o ЁEnvia lancamentos para contabilizade.                  	  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁSintaxe	 ЁCa370Incl 																  Ё╠╠
╠╠цддддддддддедддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢╠╠
╠╠ЁUso		 ЁFINA370																	  Ё╠╠
╠╠юддддддддддадддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды╠╠
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
