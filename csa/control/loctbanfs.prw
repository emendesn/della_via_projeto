...
╠╠ЁFun┤└o    ЁCTBANFS   ЁAutor  Ё Eduardo Riera         Ё Data Ё20.10.2001 Ё╠╠
...
/*/
User Function LOCTBANFS(aPergunte)
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбддддддддддд©╠╠
╠╠ЁFun┤└o    ЁMaCtbNfs  ЁAutor  Ё Eduardo Riera         Ё Data Ё20.10.2001 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддаддддддддддд╢╠╠
...
ъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъъ
/*/
User Function LOCMaCtbNfs(lDigita,lAglutina,nTpCtb,lCMV,dDataIni,dDataFim,cFilDe,cFilAte,lContNCP,oObj,lEnd)
	...
	For nY := 1 To 2
		...
		#IFDEF TOP
			If TcSrvType()<>"AS/400"
				...
				If lInterface
					If lOptimize
						oObj:IncRegua2("Executando processo de otimizacao com query") //"Executando processo de otimizacao com query"
					Else 
     					oObj:IncRegua2("Executando query") //"Executando query"
					EndIf
				EndIf
				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		...
		While ( !Eof() .And. (cAliasSF2)->F2_FILIAL == xFilial("SF2") .And.;
			...
			//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
			If cPaisLoc == "CHI"  .And. ;
				((Alltrim((cAliasSF2)->F2_TIPO) == "D" .And. Alltrim(cModulo) == "FAT") .Or.;
				(Alltrim((cAliasSF2)->F2_TIPO) <> "D" .And. Alltrim(cModulo) == "COM"))
				lValido := .F.
			EndIf
			//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			...
			If lValido
				...
				Begin Transaction
			    	If 	!lHeader
						...
						//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
						//Ё No chile contabiliza NCP separada de NF, por isto deve ser   Ё
						//Ё contabilizado com LOTE DIFERENCIADO.                         Ё
						//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
			     		If cPaisLoc == "CHI" .And. (AllTrim(cModulo) == "COM") 
...
If cPaisLoc <> "BRA" .and. SF2->(FieldPos("F2_DTDIGIT"))>0 
     ...
     Else
			If lContNCP
				U_LOCTBANCPS(lDigita,lAglutina,nTpCtb,lCMV,dDataIni,dDataFim,cFilDe,cFilAte,oObj,lEnd)
				RestArea(aAreaSM0)
				RestArea(aArea)
				cFilAnt := SM0->M0_CODFIL
				dDataBase := dSavBase
...
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддбддддддддддбдддддддбдддддддддддддддддддддддбддддддбддддддддддд©╠╠
╠╠ЁFun┤└o    ЁCTBANCPS  ЁAutor  Ё Paulo Augusto         Ё Data Ё28.04.2003 Ё╠╠
╠╠цддддддддддеддддддддддадддддддадддддддддддддддддддддддаддддддаддддддддддд╢╠╠
╠╠Ё          ЁRotina de processamento da contabilizacao off-line dos Docu- Ё╠╠
...
User Function LOCTBANCPS(lDigita,lAglutina,nTpCtb,lCMV,dDataIni,dDataFim,cFilDe,cFilAte,oObj,lEnd)
...
While ( !Eof() .And. SM0->M0_CODIGO == cEmpAnt .And. ;
	...
	If lInterface
		...
		oObj:IncRegua1("Contabilizando"+": "+SM0->M0_CODFIL+"/"+SM0->M0_FILIAL)//"Contabilizando"
	EndIf
	For nY := 1 To 2
		//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
		//Ё Processa os documentos de saida da filial corrente           Ё
		...
		#IFDEF TOP
			If TcSrvType()<>"AS/400"
				...
				If lInterface
					If lOptimize
						If GetNewPar("MV_OPTNFS",.F.) //"Executando processo de otimizacao com query"
/*					Else 
		...
		If lLctPad .And. nTpCtb == 2
			...
			dbSelectArea("SX5")
			dbSetOrder(1)
			If MsSeek(xFilial()+IIf(cPaisLoc<>"CHI","09FAT","09COM"))
				cLoteCtb := AllTrim(X5Descri())
			Else
				cLoteCtb := IIf(cPaisLoc<>"CHI","FAT ","COM ")
			EndIf		
			//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
			//Ё Executa um execblock                                         Ё
			//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				...
				Begin Transaction
			    	If 	!lHeader
						...
						Else
							cLoteCtb := "FAT "
						EndIf		
						If cPaisLoc =="CHI"	.And. (cAliasSF2)->F2_TIPO $ "D" 
							If MsSeek(xFilial()+"09COM")
								cLoteCtb := AllTrim(X5Descri())
							Else
								cLoteCtb := "COM "
...
╠╠ЁFun┤┘o    Ё CTBNFSSX1    ЁAutor Ё  Marcello            ЁDataЁ 31/07/03 Ё╠╠
