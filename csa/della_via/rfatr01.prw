...
Private nLoop2
Private nLoop3
Private nTamDet  := 13 //15         // Tamanho da Area de Detalhe Produtos
...
*/
		...
		If cTipNFi $ "N/C/P/I/S/T/O"
			...
			cInscrM		:= Iif(getmv("MV_ESTADO")=="SP",SM0->M0_INSCANT,"") // Inscricao municipal
			/*
...
If lRessar
	...
	aAdd(aParamBox,{1,"Data até",ctod("//"),"","","","",0,.T.})
	ParamBox(aParamBox,"Periodo Ressarcimento ...",@aPeriodo)
	Pergunte(cPerg,.F.)	

	cTxtRes := "Emitida para fins de ressarcimento, Artigo 270, Inciso II do RICMS aprovado pelo Decreto "
	cTxtRes += "45.490/00 periodo de "
	cTxtRes += StrZero(Day(aPeriodo[1]),2) + " de " + MesExtenso(aPeriodo[1]) + " de " + StrZero(Year(aPeriodo[1]),4) + " a "
	cTxtRes += StrZero(Day(aPeriodo[2]),2) + " de " + MesExtenso(aPeriodo[2]) + " de " + StrZero(Year(aPeriodo[2]),4)
	cTxtRes += ", valor do ressarcimento "
	cTxtRes += AllTrim(Transform(nTotFat,"@e 999,999,999,999.99"))+" ("+Extenso(nTotFat)+") ressarcimento "
	cTxtRes += "nos termos do artigo 269, Inciso I do RICMS aprovado pelo Decreto 45.490/00, emitida em "
	cTxtRes += StrZero(Day(dDatEmi),2) + " de " + MesExtenso(dDatEmi) + " de " + StrZero(Year(dDatEmi),4)+"."

	nLin ++
	cLinha  := ""
	nTamLin := 0
	For k=1 to Len(cTxtRes)
		cLinha  += Substr(cTxtRes,k,1)
		nTamLin ++
		if (nTamLin > 120 .AND. Substr(cTxtRes,k,1) = " ") .OR. k = Len(cTxtRes)
			@ nLin,005 PSAY cLinha
			nLin++
			cLinha := ""
			nTamLin := 0
		Endif
	Next
	nLin++
	@ nLin,005 PSAY "Dirceu Delamuta"
	nLin++
	@ nLIn,005 PSAY "Socio - Diretor"
	nLin++
	@ nLin,005 PSAY "CPF 420.681.348-53     RG 3.527.844-4"
Endif
