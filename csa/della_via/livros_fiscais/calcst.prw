...
*/
USER FUNCTION CALCST()
    //
	LOCAL nValor     := 0
	LOCAL nPosCOD    := round((aScan(aHeader,{|x| AllTrim(x[2]) == AllTrim("D1_COD") })),2)
	LOCAL nPsBRICMS  := round((aScan(aHeader,{|x| AllTrim(x[2]) == AllTrim("D1_BRICMS") })),2)
	LOCAL nPsBaseICM := round((aScan(aHeader,{|x| AllTrim(x[2]) == AllTrim("D1_BASEICM") })),2)
	LOCAL nPsValIPI  := round((aScan(aHeader,{|x| AllTrim(x[2]) == AllTrim("D1_VALIPI") })),2)
	LOCAL nPosTES    := round((aScan(aHeader,{|x| AllTrim(x[2]) == AllTrim("D1_TES") })),2)
	LOCAL nMargem    := 0          
	//
    If aCols[n][nPosTES]=="010" // Notas Fiscais com Substituicao
	   // Salva Areas
	   _aArea1   := GetArea()
