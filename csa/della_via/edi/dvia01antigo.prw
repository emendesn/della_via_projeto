...
While !FT_FEOF()
	...
	While !FT_FEOF() .and. cTipoSeg $ 'ITP/AE1/NF2'
		...
		If cTipoSeg == "ITP"
			...
			If aScan(_aEmpSM0,{|aVal| aVal[1] == AllTrim(cCNPJR)}) == 0 // // CNPJ do Receptor (Della Via)
				...
				GravaLog("01","CNPJ Della Via nใo cadastrado." + cCNPJR)
			...
			Else
				...
				If !dbSeek(xFilial("SA2")+cCNPJT) // CNPJ do Transmissor (Pirelli)
					...
					// Grava Log de Erro
					GravaLog("05","Fornecedor nใo cadastrado." + cCNPJR)
	...
	If dbSeek( cFilNew + M->( F1_DOC + F1_SERIE  + cForPad ) ) // NFE jแ Importada Anteriormente
        ...
        If SF1->F1_ARQEDIP <> cArqTXT
			// Grava Log de Erro
			GravaLog("02","Arquivos EDI diferem.")  // Nota em outro arquivo EDI
        Endif
	...
	Else
		...
		If dbSeek("  "+M->( F1_DOC + F1_SERIE  + cForPad ))
			...
			If UA9->UA9_STATUS == "0" // NFE Importada anteriormente com Sucesso
				...
				GravaLog("07","NFE com Stauts de Importada OK nใo cadastrada no sistema.")
...
//					Next k	
		...
		Else
			...
			For i:= 1 to Len(_aItens) 
				...
				If dbSeek(cFilAnt+M->( F1_DOC + F1_SERIE  + cForPad )+_aItens[i][3][2])
					RecLock("SD1",.F.)
					SD1->D1_PEDIDO := _aItens[i][2][2]
					SD1->D1_ITEMPC := _aItens[i][8][2]
					MsUnlock()                 
				EndIf	
			Next i
			// Grava Log Nota Fiscal Importada com Sucesso
			GravaLog("04","NFE Impotada com sucesso.")
...
If nCtErro > 0
	ApMsgStop("Houve Erros na Importacao do Arquivo.", "ATENวรO" )
	MostraErro()
EndIf
...
dbSelectArea("SA2")
dbSetOrder(3)
dbSeek(xFilial("SA2")+cCNPJT) // CNPJ do Transmissor (Pirelli)
*/                              

// Troca o produto pirelli (no .TXT - B1_CODFAB) pelo Della Via (SB1->B1_COD)
/*
dbSelectArea("SB1")
dbSetOrder(8) // B1_FILIAL+B1_CODFAB
...
dbSelectArea( "SC7" )
dbSetOrder( 4 )
dbSeek( _cNewFil +  cCod + cPedido )
// dbSeek( _cNewFil +  SB1->B1_COD + cPedido )

cRet := SC7->C7_ITEM                                      

// Troca Fornecedor/Loja pelo correto para baixa na classifica็ใo da NFE
/*
RecLock("SC7",.F.)
...
If _cCodErr $ "06"
   ...
   //	_cArqOri := NomeAutoLog()                       //Fabio
	_cArqOri := AllTrim(cArqTXT)     // NomeAutoLog()  Fabio
	
	_cArqDes := _cDir + "Log_"+SubStr(U_PegaFile( cArqTXT ),1,At(".",U_PegaFile( cArqTXT ))-1)+"_"+Dtos(date(),.f.,1)+"_"+StrTran(time(),":","")+".log" // NomeAutoLog()	
	// _cArqDes := _cDir + "Log_"+SubStr(U_PegaFile( cArqTXT ),1,At(".",U_PegaFile( cArqTXT ))-1)+"_"+Dtos(date())+"_"+StrTran(time(),":","")+".log" // NomeAutoLog()	
