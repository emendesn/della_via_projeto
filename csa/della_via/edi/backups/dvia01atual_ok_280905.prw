...
//        If !SF1->F1_ARQEDIP $ cArqTXT	// SF1->F1_ARQEDIP <> cArqTXT
			// Grava Log de Erro
			// GravaLog("02","NF jแ importada. NF/Serie/Fornec.: "+M->( F1_DOC + F1_SERIE  + cForPad ))  // Nota em outro arquivo EDI
...
//        Endif
	...
	Else
		...
		// Posiciona Status da NFE Importada
		dbSelectArea("UA9")
		dbSetOrder(3)
		If dbSeek("  "+M->( F1_DOC + F1_SERIE  + cForPad ))
		
			// Verifica Status de Importa็ใo
			If UA9->UA9_STATUS == "0" // NFE Importada anteriormente com Sucesso
		
				// Grava Log de Erro - NFE com Stauts de Importada OK nใo cadastrada no sistema
				// GravaLog("07","NFE com Stauts de Importada (OK), nใo cadastrada no sistema. NF/Fornec.: " + M->( F1_DOC + F1_SERIE  + cForPad ))
				GravaLog("07","NFE com Stauts de Importada (OK), nใo cadastrada no sistema. NF/Fornec.: " + M->( F1_DOC)+"/"+M->F1_SERIE+"-"+ Left(cForPad,6)+"/"+Right(cForPad,2))
    ...
    */
	...
	For i:= 1 to Len(aItens)     
		...
		If !dbSeek(cFilAnt+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2])		
			// GravaLog("09","NF/Serie/Fornec. " + M->F1_DOC +"/"+M->F1_SERIE+"-"+Left(cForPad,6)+"/"+Right(cForPad,2) + " com pedido de compra inexist. Pedido: " + aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+"/")			
		...
		Else
			...
			If !dbSeek(cFilAnt+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_COD"})][2]+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2])
				// Grava Log de Erro - CNPJ da Della Via nใo cadastrado
				// GravaLog("03","NF/Serie/Fornec. " + M->F1_DOC +"/"+M->F1_SERIE+"-"+Left(cForPad,6)+"/"+Right(cForPad,2) + " com produto inexistente Ped./Item/Prod.: " + aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+"/"+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2]+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_COD"})][2])			
		...
		Else
			...
			For i:= 1 to Len(_aItens) 
				...
				Else                    
					...
					// Grava Log de Erro - CNPJ da Della Via nใo cadastrado
					// GravaLog("03","NF/Serie/Fornec. " + M->F1_DOC +"/"+M->F1_SERIE+"/" + cForPad + " com ped. compra inexist. Ped./Item: " + aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+"/"+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2])
					// GravaLog("03","NF/Serie/Fornec. " + M->F1_DOC +"/"+M->F1_SERIE+"-"+Left(cForPad,6)+"/"+Right(cForPad,2) + " com ped. compra inexist. Ped./Item: " + aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+"/"+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2])
...
aAdd( aDet  , { "cQtdeCon"  , "N",   9,   9,  0, "TSC7->C7_QUANT"  } )
