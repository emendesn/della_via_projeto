...
If nOperacao == 1
	...
	Else
		ApMsgInfo("Processo de Importa็ใo Finalizado com Sucesso.", "ATENวรO")
	EndIf
Else
...
lErro :=.F. // Retorna erro na importa็ใo
	...
	For i:= 1 to Len(aItens)     
		...
		Else
			...
			If !dbSeek(cFilAnt+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_COD"})][2]+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2])
				GravaLog("03","Codigo do produto diverge da nota. Ped./Item/Prod.: " + aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+" / "+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2]+" / "+aItens[i][ASCAN(AITENS[I], { |X| X[1] == "D1_COD"})][2])			
				lContinua := .F.
				lErro:= .T.
				nCtErro++
            	Exit
				...
				// Verifica se o pedido de compra existe
 				...
 				If dbSeek(cFilAnt+_aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_COD"})][2]+_aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_PEDIDO"})][2]+_aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_ITEMPC"})][2])
					...
					RecLock("SC7",.F.)  
		 			...
		 			MsUnlock()  
		 			cComplMsgErro := ""                             
                    If SC7->C7_QUANT > _aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_QUANT"})][2]
                    	cComplMsgErro := " NF COM QTDE MENOR QUE O PEDIDO DE COMPRA! " 
                    EndIF
					If SC7->C7_QUANT < _aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_QUANT"})][2]
                    	cComplMsgErro := " NF COM QTDE MAIOR QUE O PEDIDO DE COMPRA! " 
                    EndIF  
                    If SC7->C7_PRECO > _aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_VUNIT"})][2]
                    	cComplMsgErro := cComplMsgErro + " NF COM QTDE MENOR QUE O PEDIDO DE COMPRA! " 
                    EndIF
                    If SC7->C7_PRECO > _aItens[i][ASCAN(_AITENS[I], { |X| X[1] == "D1_VUNIT"})][2]
                    	cComplMsgErro := cComplMsgErro + " NF COM QTDE MAIOR QUE O PEDIDO DE COMPRA! " 
                    EndIF
	    			...
	    			GravaLog("04","NFE Impotarda com sucesso. " + cComplMsgErro ) // Nota/Serie/Fornec.: " + M->F1_DOC +"/"+ M->F1_SERIE+"-"+Left(cForPad,6)+"/"+Right(cForPad,2) )
				Else                    
...
aAdd( aHead , { "cCodCliPir", "C",  17,  10,  0, "AllTrim(SZ1->CODPIR)" } )                   //  "SB1->B1_CODFAB"
