...
For nItem	:= 1 To Len(aDiret)
	...
	While ! FT_FEOF()
		...
		ChkFile(cAliasBusca, .T.) // forca a abertura do arquivo, caso nao esteja aberto no menu
...
For nX := 1 To Len(aAuto)
	...
	If X3Obrigat( AllTrim(aAuto[nX][1]) ) .And. Empty( AllTrim(aAuto[nX][2]) )
		lCVazio := .T.
	Else
		If SX3->(MsSeek(AllTrim(aAuto[nX][1])))
			cX3Valid := IIf(!Empty(SX3->X3_VALID),Alltrim(SX3->X3_VALID),"")
			cX3Valid += IIf(!Empty(cX3Valid),Iif(!Empty(SX3->X3_VLDUSER)," .And. ",""),"")+ Alltrim(SX3->X3_VLDUSER)
/*			
			...
			EndIf
*/
			lBlKVld  := Eval(&("{|| " + cX3Valid + "}"))
//			cBlKWhen := &("{|| " + cWhen  + "}")
		Endif
	EndIf

	AAdd(aInconsiste,{AllTrim(aAuto[nX][1]), lCVazio, lBlKVld })
Next
