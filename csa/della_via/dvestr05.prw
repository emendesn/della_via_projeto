...
User Function DVESTR05
	Private cString        := "ZX0"
	Private aOrd           := {"Codigo","Grupo"}
	Private cDesc1         := "Este programa tem como objetivo imprimir relatorio "
	Private cDesc2         := "de acordo com os parametros informados pelo usuario."
	Private cDesc3         := "Consumo/Venda NN Meses de Materiais"
	Private tamanho        := "M"
	Private nomeprog       := "DVESTR05"
	Private lAbortPrint    := .F.
	Private nTipo          := 15
	Private aReturn        := { "Zebrado", 1, "Administracao", 1, 1, 1, "", 1}
	Private nLastKey       := 0
	Private cPerg          := "ESTR05"
	Private titulo         := "Consumo/Venda NN Meses de Materiais"
	Private Li             := 80
	Private Cabec1         := ""
	Private Cabec2         := ""
	Private m_pag          := 01
	Private wnrel          := "DVESTR05"
	Private lImp           := .F.
	...
	aAdd(aRegs,{cPerg,"10","Qtd. Meses         ?"," "," ","mv_cha","N", 02,0,0,"G","(mv_par10 > 0 .and. mv_par10 < 13)","mv_par10",""       ,"","","","",""       ,"","","","",""       ,"","","","","","","","","","","","","","   ","","","","99"        })
	aAdd(aRegs,{cPerg,"11","Local              ?"," "," ","mv_chb","C", 60,0,0,"G","U_fSelLoc()"                       ,"mv_par11",""       ,"","","","",""       ,"","","","",""       ,"","","","","","","","","","","","","","   ","","","",""          })
...
Static Function RunReport()
	...
	Cabec2:="Codigo       Grupo   Descrição                        ABC    Saldo"//AI/2005  JUN/2005  JUL/2005     Media
	        *XXXXXXXXXX   XXXX    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX   X    999.999   999.999   999.999   999.999   999.999
	...
	cFilLoc  := ""
	For jj=1 to Len(AllTrim(mv_par11)) Step 2
		If Substr(mv_par11,jj,2) <> "**"
			cFilLoc := cFilLoc + "'"+Substr(mv_par11,jj,2)+"',"
		Endif
	Next jj
	If Len(cFilLoc) > 0
		...
		Do While !eof() .and. ZX0_FILIAL = cFil
			...
			Do While !eof() .and. ZX0_FILIAL = cFil .AND. &cCondSec
				...
				For k=1 to Len(aMeses)
					...
					nTotVal     += &cCampo
				Next k
				@ LI,nCol PSAY nTotVal/mv_par10 Picture "@e 999,999"
				LI++
				dbSkip()
			Enddo
