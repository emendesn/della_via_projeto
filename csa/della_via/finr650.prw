...
wnrel := "RFIR65"            //Nome Default do relatorio em Disco

aOrd  := {OemToAnsi("Numero"),OemToAnsi("Natureza"),OemToAnsi("Vencimento"),; //"Por Numero"###"Por Natureza"###"Por Vencimento"
OemToAnsi("Banco"),OemToAnsi("Fornecedor"),OemToAnsi("Emissao"),OemToAnsi("Cod.Forn")}  //"Por Banco"###"Fornecedor"###"Por Emissao"###"Por Cod.Fornec."
...
LOCAL cFilOrig := cFilAnt // Edmar em 14/3/2005
Local nCont    := 0
Local x        := 0
...
Else
	cabec1  := OemToAnsi("No.Titulo  Esp Ocorrencia  Dt.Ocor.   Desp. Cobr  Vlr Desconto  Vlr Abatimento   Vlr Principal     Vlr Juros    Vlr Multa      Nro Titulo Bco   Consistencia  Filial")
EndIf
cabec2  := ""
...
While nTamArq-nLidos >= nTamDet
	If mv_par08 == 1
		...
		EndIF
     	...
     	// Edmar em 14/3/2005
		cFilAnt := substr(xbuffer,195,2) // traz a filial do titulo
		// 
...
//						If dbSeek(subs(cnumtit,1,2)+Substr(cNumtit,3,10))
							lAchouTit := .T.
							nPos   := 1
						Endif
					Endif
/*
				Else
					// Localiza o titulo
					lAchouTit := Execblock("FR650FIL",.F.,.F.)
				Endif
*/
				While !lAchouTit
					dbSetOrder(1)
	...
	Else
		...
		If ( Empty(aLeitura[1]) )
			...
			Loop
		Endif
		cFilAnt :=  aLeitura[16] // traz a filial do titulo // Edmar em 14/3/2005
	...
	Else
		cDescr  := Space(29)
		lOcorr  := .F.
		nCntOco := Ascan(aCntOco, { |X| X[2] == OemToAnsi(" ")})
		If nCntOco == 0
			Aadd(aCntOco,{"00 ",OemToAnsi(" "),nDespes,nDescont,nAbatim,nValRec,nJuros,nMulta,nValIof,nValCc,nOutrDesp})
		Else
			aCntOco[nCntOco][DESPESAS]     +=nDespes
			aCntOco[nCntOco][DESCONTOS]    +=nDescont
			aCntOco[nCntOco][ABATIMENTOS]  +=nAbatim
	...
	@li,089 PSAY nAbatim  picture tm(nAbatim,12)  //'@E 99999,999.99'
	@li,102 PSAY nValRec  picture tm(nValRec,12)	 //'@E 99999,999.99'
	@li,115 PSAY nJuros   picture tm(nJuros,12)	 //'@E 99999,999.99'
	@li,128 PSAY nMulta   picture tm(nMulta,12)	 //'@E 99999,999.99'
	If mv_par07 == 1
		@li,141 PSAY nValIof  picture tm(nValIof,9) //'@E 99,999.99'
		@li,151 PSAY nValCc   picture tm(nValCC,12)  //'@E 99999,999.99'
		@li,164 PSAY DToC(Iif(Empty(dCred),dDataBase,dCred))
		@li,174 PSAY nOutrDesp picture tm(nOutrDesp,10)	 //'@E 99999,999.99'
		@li,184 PSAY Pad(cNossoNum,19)
	Else
		@li,140 PSAY nOutrDesp picture tm(nOutrDesp,12)	 //'@E 99999,999.99'
		@li,154 PSAY Pad(cNossoNum,19)
	EndIf
	...
	IF Empty(cOcorr)
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)  	//"OCORRENCIA NAO ENVIADA"
		lOk := ImpCons(cDescr)
	Else
		If ! lOcorr
			cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)  //"OCORRENCIA NAO ENCONTRADA"
			lOk := ImpCons(cDescr)
		End
	EndIf
	
	If dBaixa < dDataFin
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)		//"DATA MENOR QUE DATA FECH.FINANCEIRO"
		lOk := ImpCons(cDescr)
	Endif
	
	IF Empty(cNumTit)
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt) 	//"NUMERO TITULO NAO ENVIADO"
		lOk := ImpCons(cDescr)
	End
	...
	Else
		...
		cNumTit+cEspecie)
	EndIf
	If mv_par07 == 1 .And. lFr650Fil
		// Localiza o titulo pelo ponto de entrada
		lAchouTit := Execblock("FR650FIL",.F.,.F.)
		If ! lAchouTit
			cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)  	//"TITULO NAO ENCONTRADO"
			lOk := ImpCons(cDescr)
		Endif
	Else
	...
	IF Substr(dtoc(dBaixa),1,1)=' '
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt) 		//"DATA DE BAIXA NAO ENVIADA"
		lOk := ImpCons(cDescr)
	EndIF
	
	IF Empty(cTipo)
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)  	//"ESPECIE NAO ENVIADA"
		lOk := ImpCons(cDescr)
	Endif
	
	If Empty(cEspecie)
		cDescr := OemToAnsi(" ") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt) 	//"ESPECIE NAO ENCONTRADA"
		lOk := ImpCons(cDescr)
	Endif
	...
	If lOk
		If mv_par07 == 1
			If SE1->E1_SALDO = 0
				cDescr := "BAIXADO ANTERIORMENTE - TOTAL" + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)//"BAIXADO ANTERIORMENTE - TOTAL"
				lOk := ImpCons(cDescr)
			ElseIf SE1->E1_VALOR <> SE1->E1_SALDO
				cDescr := "BAIXADO ANTERIORMENTE - PARCIAL" + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt) //"BAIXADO ANTERIORMENTE - PARCIAL"
				lOk := ImpCons(cDescr)
			End
		Else
			If SE2->E2_SALDO = 0
	...
	If lOk
		cDescr := OemToAnsi("TITULO OK") + "  " + cFilAnt + " / " + SFNomeFil(cFilAnt)  	//"TITULO OK"
		lOk := ImpCons(cDescr)
	EndIf
...
IF li != 80
	...
	@li,000 PSAY OemToAnsi("SUBTOTAIS DO RELATORIO")  //"SUBTOTAIS DO RELATORIO"
	Li+=2
	For x :=1 to Len(aCntOco)
		If(x <= 10)
			...
			@li,089 PSAY (aCntOco[x][ABATIMENTOS])   picture Tm((aCntOco[x][05]),12) //'@E 9,999,999,999.99'
			@li,102 PSAY (aCntOco[x][VALORRECEBIDO]) picture Tm((aCntOco[x][06]),12) //'@E 9,999,999,999.99'
			@li,115 PSAY (aCntOco[x][JUROS])         picture Tm((aCntOco[x][07]),12) //'@E 9,999,999,999.99'
			@li,128 PSAY (aCntOco[x][MULTA])         picture Tm((aCntOco[x][08]),12) //'@E 9,999,999,999.99'
			If mv_par07 == 1
				@li,141 PSAY (aCntOco[x][VALORIOF])   picture Tm((aCntOco[x][09]),09) //'@E 9,999,999,999.99'
				@li,151 PSAY (aCntOco[x][VALORCC])    picture Tm((aCntOco[x][10]),12) //'@E 9,999,999,999.99'
				@li,174 PSAY (aCntOco[x][OUTRDESP])    picture Tm((aCntOco[x][11]),10) //'@E 9,999,999,999.99'
			Else
				@li,140 PSAY (aCntOco[x][OUTRDESP])    picture Tm((aCntOco[x][11]),10) //'@E 9,999,999,999.99'
			Endif
		Else
			Exit
		Endif
	...
	//³ Imprime Totais                ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	@li,000 PSAY OemToAnsi("TOTAIS DO RELATORIO")  //"TOTAIS DO RELATORIO"
	@li,063 PSAY nDespT      picture TM(nDespT,12) //'@E 9,999,999,999.99'
	@li,076 PSAY nDescT      picture TM(nDescT,12) //'@E 9,999,999,999.99'
	@li,089 PSAY nAbatT      picture TM(nAbatT,12) //'@E 9,999,999,999.99'
	@li,102 PSAY nValT       picture Tm(nValT,12)  //'@E 9,999,999,999.99'
	@li,115 PSAY nJurT       picture Tm(nJurT,12)  //'@E 9,999,999,999.99'
	@li,128 PSAY nMulT       picture Tm(nMult,12)  //'@E 9,999,999,999.99'
	If mv_par07 == 1
		@li,141 PSAY nIofT    picture TM(nIofT,09) //'@E 9,999,999,999.99'
		@li,151 PSAY nCcT     picture TM(nCcT,12)  //'@E 9,999,999,999.99'
		@li,174 PSAY nOut     picture TM(nOut,10)  //'@E 9,999,999,999.99'
	Else
		@li,140 PSAY nOut     picture TM(nOut,10)  //'@E 9,999,999,999.99'
	EndIf
...
cFilAnt := cFilOrig // Edmar em 14/3/2005
Return
...
Else
	@ li,177 PSAY Pad(cTexto,31)
EndIf
li++
Return .F.

STATIC FUNCTION SFNomeFil(cFilAnt)
LOCAL _aArea := GetArea()
LOCAL _cNome := SPACE(15)

DbSelectArea("SM0")
IF DbSeek(cEmpAnt+cFilAnt)
   _cNome := ALLTRIM(SM0->M0_FILIAL)
ENDIF
RestArea(_aArea)
Return(_cNome)