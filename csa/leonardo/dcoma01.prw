...
SET FILTER TO SF1->F1_TIPO ==  "B" //.AND.  //SF1->F1_FILIAL == XFILIAL("SF1")
...
While !Eof().And.(x3_arquivo=="SD1")
	If X3USO(x3_usado) .AND. Alltrim(X3_CAMPO) == "D1_ITEM" .OR.  Alltrim(X3_CAMPO) == "D1_SERIEPN" .OR. Alltrim(X3_CAMPO) == "D1_NUMFOGO" .OR. ;
		Alltrim(X3_CAMPO) == "D1_MARCAPN" .OR. Alltrim(X3_CAMPO) == "D1_CARCACA" .OR. Alltrim(X3_CAMPO) == "D1_X_DESEN" .OR. Alltrim(X3_CAMPO) == "D1_SERVICO" ;
		.OR.  Alltrim(X3_CAMPO) == "D1_TOTAL" .OR. Alltrim(X3_CAMPO) == "D1_VUNIT"  .OR. Alltrim(X3_CAMPO) == "D1_COD" .OR.   Alltrim(X3_CAMPO) == "D1_IDENTB6"
		nUsado:=nUsado+1
						...
						IF SX3->X3_CONTEXT <> "V"
							IF aScan(aHeader,{|x|Upper(AllTrim(x[2]))==Alltrim(SX3->X3_CAMPO)}) > 0
								...
								IF SC2->( dbSeek (xFilial("SC2") + _cdoc +Substr(_cItem,3,2) + '001' ) )
									IF Alltrim(SX3->X3_CAMPO) $ "D1_COD/D1_SERIEPN/D1_NUMFOGO/D1_X_DESEN/D1_CARCACA/D1_SERVICO'
										RecLock("SC2")
											...
											ElseIF Alltrim(SX3->X3_CAMPO) == "D1_COD"												
												...
												cCodAnt    := SC2->C2_PRODUTO
												SC2->C2_PRODUTO :=  aCols[i,aScan(aHeader,{|x|Upper(AllTrim(x[2]))==Alltrim(SX3->X3_CAMPO)})]												
								...
								dbSelectArea("SB6")	
								SB6->( dbSetOrder(3) )
								IF SB6->( dbSeek (xFilial("SB6") + cItemB6 )  ) //_cCod +_cFornec+_cLoja + cItemB6
									RecLock("SB6")
										...
										ElseIF Alltrim(SX3->X3_CAMPO) $  'D1_COD'
											SB6->B6_PRODUTO := aCols[i,aScan(aHeader,{|x|Upper(AllTrim(x[2]))==Alltrim(SX3->X3_CAMPO)})]	
										EndIF
									MsUnlock()
								EndIF
								IF Alltrim(SX3->X3_CAMPO) == "D1_COD"
									...
									IF dbSeek(xFilial("SD3")+ cOP)
										RecLock("SD3")
											SD3->D3_COD := aCols[i,aScan(aHeader,{|x|Upper(AllTrim(x[2]))==Alltrim(SX3->X3_CAMPO)})]
										MsUnlock()
									EndIF
								EndIF
							EndIF
