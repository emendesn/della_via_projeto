...
Local cPict        := ""
Local titulo       := "Relat๓rio de Comissใo"
Local nLin         := 80
LOCAL aPergunta         :={}
//Local Cabec1       := "Dt.Baixa Titulo Prefixo Dt.Emissao Cliente Vend/Cliente Vend.do Titulo  Valor Titulo Base Comissao  %Venda %Vendedor  Vlr.comissao OK "
//Local Cabec1       := "Dt.Baixa Titulo Prefixo Dt.Emissao Cliente Vend/Cliente Vend.do Titulo  Valor Titulo                       %Comissใo  Vlr.comissao    "
...
Local nOrdem
LOCAL aArqTrab        := {}
LOCAL nValBase  := nComissao1 := nComissao2 := nComissao3 := nPerComis1 := nPerComis2 := nPerComis3 := 0
LOCAL lDifVend1 := .F.
LOCAL lDifVend2 := .F.
LOCAL lDifVend3 := .F.
LOCAL nValComiV1:= 0
LOCAL nValComiV2:= 0
LOCAL nValComiV3:= 0
LOCAL lDifBase1  := lDifBase2  := lDifBase3  := .F.
LOCAL lDifPer1   := lDifPer2   := lDifPer3   := .F.
LOCAL aVendedores := {}
...
_cQry := "SELECT E5_DATA, E5_VALOR, E1_NUM, E1_PREFIXO, E1_EMISSAO, E1_CLIENTE, E1_LOJA, E1_VALOR, E1_VALLIQ, E1_VEND1, E1_VEND2, E1_VEND3, E1_PARCELA "
...
While !EOF()
	...
	lDifPer1 := IIF( nValComiV1 == 0, nPerComis1,nValComiV1)
	lDifPer2 := IIF( nValComiV2 == 0, nPerComis2,nValComiV2)
	lDifPer3 := IIF( nValComiV3 == 0, nPerComis3,nValComiV3)
	
	IF!Empty(TRBSE1->E1_VEND1)
		//aAdd(aVendedores,{SA1->A1_VEND,TRBSE1->E1_VEND,nPerComis1,nValComiV1,nComissao1,lDifVend1,lDifBase1,lDifPer1})
	...
	ElseIF !Empty(SA1->A1_VEND2)
		aAdd(aVendedores,{SA1->A1_VEND2,"",lDifPer1})
	EndIF
	...
	aVendedores := {}
	nPerComis1 := nPerComis2 := nPerComis3 := 0
	nValComiV1 := nValComiV2 := nValComiV3 := 0
	nComissao1 := nComissao2 := nValComiV3 := nValBase:= 0
	lDifVend1  := lDifVend2  := lDifVend3  := .F.
	lDifBase1  := lDifBase2  := lDifBase3  := .F.
	lDifPer1   := lDifPer2   := lDifPer3   := .F.
